...
@section('custom-script')
    ...
    <script>
        ...
        function changeUrl() {
            var nam = $('#namhs').val();
            var url = '/kekhaigiadvcahue?&madv=' + $('#madv').val() + '&nam=' + nam + '&trangthai=' + $('#trangthai').val();
...
@section('content')
    <h3 class="page-title">
        Thông tin kê khai giá<small>&nbsp;dịch vụ ca huế</small>
        <p>
                ...
                <div class="portlet-title">
                    <div class="actions">
                        @if (!empty($inputs['madv']) && $inputs['madv'] != 'ALL')
                            {
                            <a href="{{ url('kekhaigiadvcahue/create?&madv=' . $inputs['madv']) }}"
                        ...
                        <tbody>
                            @foreach ($model as $key => $tt)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ url('kekhaigiadvcahue/prints?&mahs=' . $tt->mahs) }}" target="_blank"
                                            class="btn btn-default btn-xs mbs"><i class="fa fa-eye"></i>&nbsp;Xem chi
                                            tiết</a>
                                        @if (canEdit($tt->trangthai))
                                            <a href="{{ url('kekhaigiadvcahue/edit?mahs=' . $tt->mahs) }}"
    ...
    <div class="modal fade" id="delete-modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                {!! Form::open(['url' => 'kekhaigiadvcahue/delete', 'id' => 'frm_delete']) !!}
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
