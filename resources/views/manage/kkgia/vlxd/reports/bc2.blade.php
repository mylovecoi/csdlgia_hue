...
<p style="text-align: center; font-weight: bold; font-size: 16px;text-transform: uppercase;">BÁO CÁO KÊ KHAI Giá vật liệu xây dựng</p>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="active" colspan="8">{{$tt->tendn}}
                ...
                -<b>Mã hồ sơ:</b> <a href="{{ url('kekhaigiavlxd/prints?mahs=' . $tt->mahs) }}">
                    {{$tt->mahs}}
                </a>
                . Số công văn :{{$tt->socv}} -
                Ngày hiệu lực: {{getDayVn($tt->ngayhieuluc)}}
                - Ngày chuyển: {{getDateTime($tt->ngaychuyen)}}
                <br>Số hồ sơ nhận: {{$tt->sohsnhan}} - Ngày nhận: {{getDayVn($tt->ngaynhan)}}
            </td>
        </tr>
        ...
        <tr>
            ...
            <td style="text-align: center">{{$tt->tendvcu}}</td>
            <td style="text-align: center">{{$tt->dvt}}</td>
            <td style="text-align: right">{{number_format($tt->gialk)}}</td>
            <td style="text-align: right">{{number_format($tt->giakk)}}</td>
            <td style="text-align: right">
                {{--@if($tt->giakk > $tt->gialk)
                            Tăng : {{($tt->giakk) - ($tt->gialk)}}
                @elseif($tt->giakk < $tt->gialk)
                    Giảm : {{($tt->giakk) - ($tt->gialk)}}
                    @else
                    Giữ nguyên : {{($tt->giakk) - ($tt->gialk)}}
                    @endif--}}
                    {{number_format(($tt->giakk) - ($tt->gialk))}}
            </td>
        </tr>
        @endforeach
        @endforeach
