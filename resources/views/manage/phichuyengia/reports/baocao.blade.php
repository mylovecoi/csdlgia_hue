...
@section('content')
    ...
    <p style="text-align: center;font-weight: bold;font-size: 14px;text-transform:uppercase;">
        Hồ sơ giá sản phẩm, dịch vụ công ích, dịch vụ sự nghiệp công và hàng hóa, dịch vụ được địa phương đặt hàng, giao kế hoạch sản xuất, kinh doanh
    </p>
    ...
    <table cellspacing="0" cellpadding="0" border="1" style="margin: 20px auto; border-collapse: collapse;">
        <tr>
            ...
            <th style="text-align: center">Tên giá</th>
            <th style="text-align: center" width="10%">Đơn vị<br>tính</th>
            <th style="text-align: center" width="15%">Mức giá</th>
        </tr>
        @foreach($model as $key=>$tt)
            <tr id={{$tt->id}}>
                <td style="text-align: center">{{($key +1)}}</td>
                <td class="active" style="font-weight: bold">{{$tt->tenphi}}</td>
                <td class="active" style="font-weight: bold">{{$tt->tengia}}</td>
                <td style="text-align: left;">{{$tt->dvt}}</td>
                <td style="text-align: right;font-weight: bold">{{dinhdangso($tt->mucgia)}}</td>
            </tr>
        @endforeach
