...
@section('custom-script-cb')
    ...
    <script>
        ...
        function changeUrl() {
                ...
                var url = current_path_url + 'nam=' + escapeHtml($('#nam').val()) + '&manghe=' + escapeHtml($(
                    '#manghe').val());
                ...
                <div class="portlet box">
                    <div class="portlet-title">
                        <div class="caption text-uppercase">
                            <span
                                class="caption-subject theme-font bold uppercase">{{ session('congbo')['chucnang']['bog'] ?? 'Bình ổn giá' }}</span>
                        </div>
                    </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-control select2me" name="manghe" id="manghe">
                                    ...
                                    @foreach ($a_bog as $key => $val)
                                        <option {{ $key == $inputs['manghe'] ? 'selected' : '' }} value="{{ $key }}">
                                            {{ $val }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <tbody>
                                @foreach ($model as $key => $tt)
                                    <tr>
                                        ...
                                        <td style="text-align: right;font-weight: bold">{{ number_format($tt->giakk) }}
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
