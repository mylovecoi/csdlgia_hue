...
class GiaThueMuaNhaXhController extends Controller
{
    public function index(Request $request)
    {
        ...
        if (Session::has('admin')) {
            ...
            $m_donvi = getDonViNhapLieu(session('admin')->level,'giathuemuanhaxh');
            if (count($m_donvi) == null) {
                $message = 'Chưa có đơn vị nào được phân quyền nhập liệu cho chức năng: ' . session('admin')['a_chucnang']['giathuemuanhaxh']
                    . '. Bạn cần liên hệ người quản trị để phần quyền nhập liệu cho đơn vị.';
                return  view('errors.403')
                    ->with('message', $message);
            }
            $m_donvi_th = getDonViTongHop('giathuemuanhaxh',\session('admin')->level, \session('admin')->madiaban);
            $inputs['madiaban'] = $inputs['madiaban'] ?? $m_diaban->first()->madiaban;
            $inputs['madv'] = $inputs['madv'] ?? $m_donvi->first()->madv;
            $inputs['nam'] = $inputs['nam'] ?? 'all';

            //lấy thông tin đơn vị
            $model = GiaThueMuaNhaXh::where('madv', $inputs['madv']);
            if ($inputs['nam'] != 'all')
    ...
    public function destroy(Request $request){
        if (Session::has('admin')) {
            $inputs = $request->all();
            $model = GiaThueMuaNhaXh::where('mahs', $inputs['mahs'])->first();
            $model->delete();
            return redirect('thuemuanhaxahoi/danhsach?madv=' . $model->madv);
        } else
            return view('errors.notlogin');
    }
...
//            $model->save();
    ...
    public function edit(Request $request){
        ...
        if(Session::has('admin')){
            ...
            $model = GiaThueMuaNhaXh::where('mahs',$inputs['mahs'])->first();
            $modelct = GiaThueMuaNhaXhCt::where('mahs',$model->mahs)->get();
            $inputs['url'] = '/thuemuanhaxahoi';
            $a_dm = array_column(dmnhaxh::all()->toArray(),'tennha','maso');
            $a_diaban = getDiaBan_Level(\session('admin')->level, \session('admin')->madiaban);
            $m_diaban = dsdiaban::wherein('madiaban', array_keys($a_diaban))->where('level','H')->get();
    ...
    public function update(Request $request)
    {
        if (Session::has('admin')) {
            ...
            if(isset($inputs['ipf1'])){
                ...
                $ipf1->move(public_path() . '/data/thuemuanhaxahoi/', $name);
                $inputs['ipf1']= $name;
            }
            if(isset($inputs['ipf2'])){
                $ipf2 = $request->file('ipf2');
                $name = $inputs['mahs'] .'&2.'.$ipf2->getClientOriginalName();
                $ipf2->move(public_path() . '/data/thuemuanhaxahoi/', $name);
                $inputs['ipf2']= $name;
            }
            if(isset($inputs['ipf3'])){
                $ipf3 = $request->file('ipf3');
                $name = $inputs['mahs'] .'&3.'.$ipf3->getClientOriginalName();
                $ipf3->move(public_path() . '/data/thuemuanhaxahoi/', $name);
                $inputs['ipf3']= $name;
            }
            if(isset($inputs['ipf4'])){
                $ipf4 = $request->file('ipf4');
                $name = $inputs['mahs'] .'&4.'.$ipf4->getClientOriginalName();
                $ipf4->move(public_path() . '/data/thuemuanhaxahoi/', $name);
                $inputs['ipf4']= $name;
            }
            if(isset($inputs['ipf5'])){
                $ipf5 = $request->file('ipf5');
                $name = $inputs['mahs'] .'&5.'.$ipf5->getClientOriginalName();
                $ipf5->move(public_path() . '/data/thuemuanhaxahoi/', $name);
                $inputs['ipf5']= $name;
            }

            $chk = GiaThueMuaNhaXh::where('mahs', $inputs['mahs'])->first();
            //dd($chk);
            if ($chk == null) {
                $inputs['trangthai'] = 'CHT';
                GiaThueMuaNhaXh::create($inputs);
            } else {
                $chk->update($inputs);
            }

            return redirect('thuemuanhaxahoi/danhsach?&madv=' . $inputs['madv']);
        } else
            return view('errors.notlogin');
    }
    ...
    public function show_dk(Request $request)
    {
        ...
        if (isset($model->ipf1)) {
            ...
            $result['message'] .= '<p><a target = "_blank" href = "' . url('/data/thuemuanhaxahoi/' . $model->ipf1) . '">' . $model->ipf1 . '</a ></p>';
            $result['message'] .= '</div></div>';
        }
        if (isset($model->ipf2)) {
            $result['message'] .= '<div class="col-md-6" ><div class="form-group">';
            $result['message'] .= '<label class="control-label" > File đính kèm</label>';
            $result['message'] .= '<p><a target = "_blank" href = "' . url('/data/thuemuanhaxahoi/' . $model->ipf2) . '">' . $model->ipf2 . '</a ></p>';
            $result['message'] .= '</div></div>';
        }
        if (isset($model->ipf3)) {
            $result['message'] .= '<div class="col-md-6" ><div class="form-group">';
            $result['message'] .= '<label class="control-label" > File đính kèm</label>';
            $result['message'] .= '<p><a target = "_blank" href = "' . url('/data/thuemuanhaxahoi/' . $model->ipf3) . '">' . $model->ipf3 . '</a ></p>';
            $result['message'] .= '</div></div>';
        }
        if (isset($model->ipf4)) {
            $result['message'] .= '<div class="col-md-6" ><div class="form-group">';
            $result['message'] .= '<label class="control-label" > File đính kèm</label>';
            $result['message'] .= '<p><a target = "_blank" href = "' . url('/data/thuemuanhaxahoi/' . $model->ipf4) . '">' . $model->ipf4 . '</a ></p>';
            $result['message'] .= '</div></div>';
        }
        if (isset($model->ipf5)) {
            $result['message'] .= '<div class="col-md-6" ><div class="form-group">';
            $result['message'] .= '<label class="control-label" > File đính kèm</label>';
            $result['message'] .= '<p><a target = "_blank" href = "' . url('/data/thuemuanhaxahoi/' . $model->ipf5) . '">' . $model->ipf5 . '</a ></p>';
            $result['message'] .= '</div></div>';
        }
        $result['message'] .= '</div>';
    ...
    public function xetduyet(Request $request)
    {
        ...
        if (Session::has('admin')) {
            ...
            $m_donvi = getDonViXetDuyet(session('admin')->level,'giathuemuanhaxh');
            $m_donvi_th = getDonViTongHop('giathuemuanhaxh',\session('admin')->level, \session('admin')->madiaban);
            $inputs['madiaban'] = $inputs['madiaban'] ?? $m_diaban->first()->madiaban;
            $inputs['madv'] = $inputs['madv'] ?? $m_donvi->first()->madv;
            $inputs['nam'] = $inputs['nam'] ?? 'all';
            ...
            switch ($inputs['level']){
                case 'H':{
                    $model = GiaThueMuaNhaXh::where('madv_h', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
                ...
                case 'T':{
                    $model = GiaThueMuaNhaXh::where('madv_t', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
                ...
                case 'ADMIN':{
                    $model = GiaThueMuaNhaXh::where('madv_ad', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
            ...
            //dd($model);
            $a_nhaxh = array_column(dmnhaxh::all()->toArray(),'tennha','maso');
            return view('manage.dinhgia.giathuemuanhaxh.xetduyet.index')
                ->with('model', $model)
                ->with('inputs', $inputs)
                ->with('m_diaban', $m_diaban)
                ->with('a_nhaxh', $a_nhaxh)
    ...
    public function chuyenxd(Request $request)
    {
        ...
        if (Session::has('admin')) {
            ...
            //dd($model);
            $model->save();
            return redirect('thuemuanhaxahoi/xetduyet?madv=' . $inputs['madv']);
        } else
            return view('errors.notlogin');
    }
    ...
    public function congbo(Request $request)
    {
        if (Session::has('admin')) {
            ...
            $model->save();
            return redirect('thuemuanhaxahoi/xetduyet?madv=' . $model->madv_ad);
        } else
            return view('errors.notlogin');
    }
    //</editor-fold>
    ...
    public function timkiem(){
        if(Session::has('admin')){
            $a_diaban = getDiaBan_Level(\session('admin')->level, \session('admin')->madiaban);
            $m_diaban = dsdiaban::wherein('madiaban', array_keys($a_diaban))->get();
            $m_donvi = getDonViTimKiem(session('admin')->level, \session('admin')->madiaban,'giathuemuanhaxh');
            //dd($m_diaban);
            $a_nhaxh = array_column(dmnhaxh::all()->toArray(),'tennha','maso');
    ...
    public function ketquatk(Request $request){
        if(Session::has('admin')){
            ...
            $inputs = $request->all();
            $m_donvi = getDonViTimKiem(session('admin')->level, \session('admin')->madiaban,'giathuemuanhaxh');
            $model = view_giathuemuanhaxh::wherein('madv',array_column($m_donvi->toarray(),'madv'));
            //dd($inputs);
            ...
            if($inputs['maso'] != 'all'){
                $model = $model->where('maso',$inputs['maso']);
            }
            ...
            $a_nhaxh = array_column(dmnhaxh::all()->toArray(),'tennha','maso');
            return view('manage.dinhgia.giathuemuanhaxh.timkiem.result')
                ->with('model',$model->get())
                ->with('a_diaban',array_column($m_donvi->toarray(),'tendiaban','madiaban'))
                ->with('a_donvi',array_column($m_donvi->toarray(),'tendv','madv'))
                ->with('a_nhaxh',$a_nhaxh)
                ->with('pageTitle','Tìm kiếm thông tin giá thuê, thuê mua nhà xã hội');
        }else
            return view('errors.notlogin');
    }
    ...
    function getHoSo($inputs)
    {
        ...
        switch ($inputs['level']) {
            case 'H':
            {
                $model = view_giathuemuanhaxh::where('madv_h', $inputs['madv']);
                if ($inputs['nam'] != 'all')
                    $model = $model->whereYear('thoidiem_h', $inputs['nam']);
                break;
            }
            case 'T':
            {
                $model = view_giathuemuanhaxh::where('madv_t', $inputs['madv']);
                if ($inputs['nam'] != 'all')
                    $model = $model->whereYear('thoidiem_t', $inputs['nam']);
                break;
            }
            case 'ADMIN':
            {
                $model = view_giathuemuanhaxh::where('madv_ad', $inputs['madv']);
                if ($inputs['nam'] != 'all')
                    $model = $model->whereYear('thoidiem_ad', $inputs['nam']);
                break;
            }
