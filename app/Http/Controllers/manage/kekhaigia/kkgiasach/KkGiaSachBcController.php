...
class KkGiaSachBcController extends Controller
{
    public function index(){
        if (Session::has('admin')) {
            ...
            return view('manage.kkgia.sach.reports.index')
                ...
                ->with('pageTitle', 'Báo cáo tổng hợp kê khai giá sach');
        }else
    ...
    public function bc1(Request $request){
        if (Session::has('admin')) {
            $inputs = $request->all();
            $model =  KkGiaSach::join('company','company.madv','=','kkgiasach.madv')
    ...
    public function bc2(Request $request){
        if (Session::has('admin')) {
            ...
            $modelct = KkGiaSachCt::whereIn('mahs',explode(',',$mahss))->get();
