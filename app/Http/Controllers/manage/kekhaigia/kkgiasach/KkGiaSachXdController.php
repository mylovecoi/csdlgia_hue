...
class KkGiaSachXdController extends Controller
{
    public function index(Request $request)
    {
        if (Session::has('admin')) {
            ...
            switch ($inputs['level']){
                case 'H':{
                    $model = KkGiaSach::where('madv_h', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
                ...
                case 'T':{
                    $model = KkGiaSach::where('madv_t', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
                ...
                case 'ADMIN':{
                    $model = KkGiaSach::where('madv_ad', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
            ...
            return view('manage.kkgia.sach.kkgia.xetduyet.index')
                ...
                ->with('pageTitle', 'Xét duyệt hồ sơ kê khai giá sách');
        } else
            return view('errors.notlogin');
    }

    public function ttdnkksach(Request $request){
        $result = array(
            'status' => 'fail',
            'message' => 'error',
        );
        ...
        if(isset($inputs['id'])){
            ...
            $modelhs = KkGiaSach::where('id',$inputs['id'])->first();
    ...
    public function tralai(Request $request){
        if (Session::has('admin')) {
            ...
            return redirect('xetduyetgiasach?madv=' . $inputs['madvtralai']);
    ...
    public function ttnhanhs(Request $request)
    {
        ...
        $inputs = $request->all();
        $modelhs = KkGiaSach::where('mahs', $inputs['mahs'])->first();
        $modeldn = Company::where('madv', $modelhs->madv)->first();
    ...
    public function getsohsnhan($macqcq){
        $idmax = KkGiaSach::wherein('trangthai', ['DD', 'CB', 'HCB'])
            ->max('id');
        if (isset($idmax)) {
            $model = KkGiaSach::where('id',$idmax)->first();
    ...
    public function nhanhs(Request $request){
        if (Session::has('admin')) {
            ...
            return redirect('xetduyetgiasach?madv='.$model->macqcq);
        }else
            return view('errors.notlogin');
    }
    ...
    public function congbo(Request $request)
    {
        if (Session::has('admin')) {
            ...
            $model->save();
            return redirect('xetduyetgiasach?madv=' . $model->madv_ad);
        } else
            return view('errors.notlogin');
    }
