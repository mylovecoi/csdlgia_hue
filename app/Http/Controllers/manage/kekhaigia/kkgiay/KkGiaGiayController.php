...
class KkGiaGiayController extends Controller
{
        ...
        if (Session::has('admin')) {
            if (session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X') {
                ...
                return view('manage.kkgia.giay.kkgia.kkgiadv.ttdn')
                    ...
                    ->with('inputs', $inputs)
                    ->with('ttql', $ttql)
                    ->with('pageTitle', 'Danh sách doanh nghiệp kê khai giá giấy');
            } else {
                return view('errors.perm');
            }
        } else
    ...
    public function index(Request $request)
    {
        if (Session::has('admin')) {
            $inputs = $request->all();
            $inputs['url'] = '/kekhaigiagiay';
            $m_donvi = getDoanhNghiepNhapLieu(session('admin')->level, 'GIAY');
            if (count($m_donvi) == 0) {
                return view('errors.noperm')
                    ->with('url', '')
                    ->with('message', 'Hệ thống chưa có doanh nghiệp kê khai giá giấy.');
            }
            ...
            $m_donvi_th = getDonViTongHop_dn('giay', session('admin')->level, session('admin')->madiaban);
            ...
            return view('manage.kkgia.giay.kkgia.kkgiadv.index')
                ...
                ->with('pageTitle', 'Danh sách hồ sơ kê khai giá giấy');
        } else
            return view('errors.notlogin');
    }
    ...
    public function create(Request $request)
    {
        if (Session::has('admin')) {
            ...
            $modellk = KkGiaGiay::where('madv', $inputs['madv'])
                ->wherein('trangthai', ['DD', 'CB', 'HCB'])
                ->orderby('ngayhieuluc', 'desc')->first();

            if ($modellk != null) {
                $modellkct = KkGiaGiayCt::where('mahs', $modellk->mahs)->get();
                ...
                KkGiaGiayCt::insert($a_dm);
            }

            $modelct = KkGiaGiayCt::where('mahs', $inputs['mahs'])->get();
            //            dd($modelct);
            ...
            return view('manage.kkgia.giay.kkgia.kkgiadv.edit')
                ...
                ->with('inputs', $inputs)
                ->with('pageTitle', 'Kê khai giá giấy thêm mới');
        } else
            return view('errors.notlogin');
    }
    ...
    public function store(Request $request)
    {
        if (Session::has('admin')) {
            ...
            return redirect('kekhaigiagiay?&madv=' . $inputs['madv']);
        } else
            return view('errors.notlogin');
    }
    ...
    public function show(Request $request)
    {
        if (Session::has('admin')) {
            ...
            if (strtotime($modelkk->ngayhieuluc) < strtotime('2024-01-01')) {
                return view('manage.kkgia.giay.reports.print56')
                ->with('modelkk', $modelkk)
                ->with('modeldn', $modeldn)
                ->with('modelkkct', $modelkkct)
                ->with('modelcqcq', $modelcqcq)
                ->with('pageTitle', 'Kê khai giá giấy');
            }
        ...
        if (Session::has('admin')) {
            if (session('admin')->level == 'DN' || session('admin')->level == 'T' || session('admin')->level == 'H'  || session('admin')->level == 'X') {
                ...
                if (session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X' || $model->madv == session('admin')->madv) {
                    ...
                    return redirect('kekhaigiagiay?&madv=' . $model->madv);
                } else
                    return view('errors.perm');
            } else
                return view('errors.perm');
    ...
    public function delete(Request $request)
    {
        if (Session::has('admin')) {
            if (session('admin')->level == 'DN' || session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X') {
                $inputs = $request->all();
                $model = KkGiaGiay::where('id', $inputs['iddelete'])
                    ->first();
                if ($model->delete()) {
    ...
    public function kiemtra(Request $request)
    {
        ...
        //dd($request);
        $inputs = $request->all();
        $m_hs = KkGiaGiay::where('mahs', $inputs['mahs'])->first();
        if (KiemTraNgayApDung($m_hs->ngayhieuluc, 'giay')) {
            $result = array(
                'status' => 'success',
