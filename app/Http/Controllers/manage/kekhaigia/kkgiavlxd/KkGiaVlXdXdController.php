...
class KkGiaVlXdXdController extends Controller
{
    public function index(Request $request)
    {
        if (Session::has('admin')) {
            ...
            switch ($inputs['level']){
                case 'H':{
                    $model = KkGiaVlXd::where('madv_h', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
                ...
                case 'T':{
                    $model = KkGiaVlXd::where('madv_t', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
                ...
                case 'ADMIN':{
                    $model = KkGiaVlXd::where('madv_ad', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
    ...
    public function ttdnvlxd(Request $request){
        $result = array(
            'status' => 'fail',
            'message' => 'error',
        );
        ...
        if(isset($inputs['id'])){
            ...
            $modelhs = KkGiaVlXd::where('id',$inputs['id'])->first();
    ...
    public function tralai(Request $request){
        if (Session::has('admin')) {
            ...
            return redirect('xetduyetkkgiavlxd?madv=' . $inputs['madvtralai']);
    ...
    public function ttnhanhs(Request $request)
    {
        ...
        $inputs = $request->all();
        $modelhs = KkGiaVlXd::where('mahs', $inputs['mahs'])->first();
        $modeldn = Company::where('madv', $modelhs->madv)->first();
    ...
    public function getsohsnhan($macqcq){
        $idmax = KkGiaVlXd::wherein('trangthai', ['DD', 'CB', 'HCB'])
            ->max('id');
        if (isset($idmax)) {
            $model = KkGiaVlXd::where('id',$idmax)->first();
    ...
    public function nhanhs(Request $request){
        if (Session::has('admin')) {
            ...
            return redirect('xetduyetkkgiavlxd?madv='.$model->macqcq);
        }else
            return view('errors.notlogin');
    }
    ...
    public function congbo(Request $request)
    {
        if (Session::has('admin')) {
            ...
            $model->save();
            return redirect('xetduyetkkgiavlxd?madv=' . $model->madv_ad);
        } else
            return view('errors.notlogin');
    }
    ...
    public function search(Request $request){
        if (Session::has('admin')) {
            ...
            $model = KkGiaVlXdCt::join('kkgiavlxd','kkgiavlxd.mahs','=','kkgiavlxdct.mahs')
                ->join('company','company.madv','=','kkgiavlxd.madv')
                ->select('kkgiavlxdct.*','company.tendn','kkgiavlxd.ngayhieuluc')
                ->where('kkgiavlxd.trangthai','DD');
            if($inputs['mota'] != '')
                $model = $model->where('kkgiavlxdct.tendvcu','like','%'.$inputs['mota'].'%');
            if($inputs['nam'] != 'all')
                $model = $model->whereYear('kkgiavlxd.ngayhieuluc',$inputs['nam']);
            $model = $model->get();
            /*dd($model);*/
    ...
    public function printf(Request $request){
        if (Session::has('admin')) {
            ...
            $model = $model->get();
            /*dd($model);*/
            return view('manage.kkgia.vlxd.kkgia.timkiem.printf')
                ->with('model',$model)
                ->with('inputs',$inputs)
                ->with('pageTitle','Tìm kiếm thông tin kê khai giá vật liệu xây dựng');
        }else
