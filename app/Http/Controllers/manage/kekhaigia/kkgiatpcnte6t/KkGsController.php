...
class KkGsController extends Controller
{
    public function index(Request $request)
    {
        if (Session::has('admin')) {
            $inputs = $request->all();
            $inputs['url'] = '/kekhaigiatpcnte6t';
            $m_donvi = getDoanhNghiepNhapLieu(session('admin')->level, 'TPCNTE6T');
            if (count($m_donvi) == 0) {
                return view('errors.noperm')
                    ->with('url', '')
                    ->with('message', 'Hệ thống chưa có doanh nghiệp kê khai giá thực phẩm chức năng cho trẻ em dưới 6 tuổi.');
            }
            ...
            $model = KkGs::where('madv', $inputs['madv'])
            ...
            $m_donvi_th = getDonViTongHop_dn('tpcnte6t', session('admin')->level, session('admin')->madiaban);
            ...
            return view('manage.kkgia.dvgs.kkgia.kkgiadv.index')
                ...
                ->with('pageTitle', 'Danh sách hồ sơ kê khai giá thực phẩm chức năng cho trẻ em dưới 6 tuổi');
        } else
    ...
    public function create(Request $request)
    {
        if (Session::has('admin')) {
            ...
            $modellk = KkGs::where('madv', $inputs['madv'])
                ->wherein('trangthai', ['DD', 'CB', 'HCB'])
                ->orderby('ngayhieuluc', 'desc')->first();

            if ($modellk != null) {
                $modellkct = KkGsCt::where('mahs', $modellk->mahs)->get();
            ...
            $modelct = KkGsCt::where('mahs', $inputs['mahs'])->get();
            //            dd($model);
            ...
            return view('manage.kkgia.dvgs.kkgia.kkgiadv.edit')
                ...
                ->with('inputs', $inputs)
                ->with('pageTitle', 'Kê khai giá thực phẩm chức năng cho trẻ em dưới 6 tuổi thêm mới');
        } else
            return view('errors.notlogin');
    }
    ...
    public function store(Request $request)
    {
        if (Session::has('admin')) {
            ...
            $model = KkGs::where('mahs', $inputs['mahs'])->first();
            if ($model == null) {
                $inputs['trangthai'] = 'CC';
                KkGs::create($inputs);
            } else {
            ...
            return redirect('kekhaigiatpcnte6t?&madv=' . $inputs['madv']);
        } else
            return view('errors.notlogin');
    }
    ...
    public function edit(Request $request)
    {
        if (Session::has('admin')) {
            ...
            $modelct = KkGsCt::where('mahs', $model->mahs)->get();
            return view('manage.kkgia.dvgs.kkgia.kkgiadv.edit')
                ->with('model', $model)
                ->with('modeldn', $modeldn)
                ->with('modelct', $modelct)
        ...
        if (Session::has('admin')) {
            if (session('admin')->level == 'DN' || session('admin')->level == 'T' || session('admin')->level == 'H'  || session('admin')->level == 'X') {
                ...
                if (session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X' || $model->madv == session('admin')->madv) {
                    ...
                    return redirect('kekhaigiatpcnte6t?&madv=' . $model->madv);
                } else
    ...
    public function kiemtra(Request $request)
    {
        ...
        //dd($request);
        $inputs = $request->all();
        $m_hs = KkGs::where('mahs', $inputs['mahs'])->first();
        if (KiemTraNgayApDung($m_hs->ngayhieuluc, 'tpcnte6t')) {
            $result = array(
                'status' => 'success',
    ...
    public function create_excel(Request $request)
    {
        if (Session::has('admin')) {
            ...
            return view('manage.kkgia.dvgs.kkgia.kkgiadv.edit')
                ...
                ->with('modelct', $modelct)
                ->with('inputs', $inputs)
                ->with('pageTitle', 'Kê khai giá thực phẩm chức năng cho trẻ em thêm mới');
        } else
            return view('errors.notlogin');
    }
}
