...
class KkGsXdController extends Controller
{
    public function index(Request $request)
    {
        if (Session::has('admin')) {
            $inputs = $request->all();
            $inputs['url'] = '/xetduyetkkgiatpcnte6t';
            //lấy địa bàn
            ...
            switch ($inputs['level']){
                case 'H':{
                    $model = KkGs::where('madv_h', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
                ...
                case 'T':{
                    $model = KkGs::where('madv_t', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
                ...
                case 'ADMIN':{
                    $model = KkGs::where('madv_ad', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
            ...
            return view('manage.kkgia.dvgs.kkgia.xetduyet.index')
                ...
                ->with('pageTitle', 'Xét duyệt hồ sơ kê khai giá thực phẩm chức năng cho trẻ em dưới 6 tuổi');
        } else
            return view('errors.notlogin');
    }

    public function ttdnkktpcnte6t(Request $request){
        $result = array(
            'status' => 'fail',
            'message' => 'error',
        );
        ...
        if(isset($inputs['id'])){
            ...
            $modelhs = KkGs::where('id',$inputs['id'])->first();
    ...
    public function tralai(Request $request){
        if (Session::has('admin')) {
            ...
            return redirect('xetduyetkkgiatpcnte6t?madv=' . $inputs['madvtralai']);
    ...
    public function ttnhanhs(Request $request)
    {
        ...
        $inputs = $request->all();
        $modelhs = KkGs::where('mahs', $inputs['mahs'])->first();
        $modeldn = Company::where('madv', $modelhs->madv)->first();
    ...
    public function getsohsnhan($macqcq){
        $idmax = KkGs::wherein('trangthai', ['DD', 'CB', 'HCB'])
            ->max('id');
        if (isset($idmax)) {
            $model = KkGs::where('id',$idmax)->first();
    ...
    public function nhanhs(Request $request){
        if (Session::has('admin')) {
            ...
            return redirect('xetduyetkkgiatpcnte6t?madv='.$model->macqcq);
        }else
            return view('errors.notlogin');
    }
    ...
    public function congbo(Request $request)
    {
        if (Session::has('admin')) {
            ...
            $model->save();
            return redirect('xetduyetkkgiatpcnte6t?madv=' . $model->madv_ad);
        } else
            return view('errors.notlogin');
    }
    ...
    public function search(Request $request){
        if (Session::has('admin')) {
            ...
            $model = KkGsCt::join('kkgs','kkgs.mahs','=','kkgsct.mahs')
                ->join('company','company.madv','=','kkgs.madv')
                ->select('kkgsct.*','company.tendn','kkgs.ngayhieuluc')
                ->where('kkgs.trangthai','DD');
            if($inputs['mota'] != '')
                $model = $model->where('kkgsct.tendvcu','like','%'.$inputs['mota'].'%');
            if($inputs['nam'] != 'all')
                $model = $model->whereYear('kkgs.ngayhieuluc',$inputs['nam']);
            $model = $model->get();
            /*dd($model);*/
            return view('manage.kkgia.dvgs.kkgia.timkiem.index')
                ->with('model',$model)
                ->with('inputs',$inputs)
                ->with('pageTitle','Tìm kiếm thông tin kê khai giá thực phẩm chức năng cho trẻ em dưới 6 tuổi');
        }else
            return view('errors.notlogin');
    }
    ...
    public function printf(Request $request){
        if (Session::has('admin')) {
            ...
            $model = $model->get();
            /*dd($model);*/
            return view('manage.kkgia.dvgs.kkgia.timkiem.printf')
                ->with('model',$model)
                ->with('inputs',$inputs)
                ->with('pageTitle','Tìm kiếm thông tin kê khai giá thực phẩm chức năng cho trẻ em dưới 6 tuổi');
        }else
