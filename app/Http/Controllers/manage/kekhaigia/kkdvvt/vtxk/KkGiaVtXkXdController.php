...
class KkGiaVtXkXdController extends Controller
{
    public function index(Request $request)
    {
        ...
        if (Session::has('admin')) {
            $inputs = $request->all();
            $inputs['url'] = '/xetduyetkekhaigiavtxk';
            //lấy địa bàn
            ...
            switch ($inputs['level']){
                case 'H':{
                    $model = GiaVtXk::where('madv_h', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
                ...
                case 'T':{
                    $model = GiaVtXk::where('madv_t', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
                ...
                case 'ADMIN':{
                    $model = GiaVtXk::where('madv_ad', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
    ...
    public function ttdnkkvtxk(Request $request){
        ...
        if(isset($inputs['id'])){
            ...
            $modelhs = GiaVtXk::where('id',$inputs['id'])->first();
    ...
    public function tralai(Request $request){
        if (Session::has('admin')) {
            ...
            return redirect('xetduyetkekhaigiavtxk?madv=' . $inputs['madvtralai']);
    ...
    public function ttnhanhs(Request $request)
    {
        ...
        //dd($request);
        $inputs = $request->all();
        $modelhs = GiaVtXk::where('mahs', $inputs['mahs'])->first();
        $modeldn = Company::where('madv', $modelhs->madv)->first();
    ...
    public function getsohsnhan($macqcq){
        $idmax = GiaVtXk::wherein('trangthai', ['DD', 'CB', 'HCB'])
            //->where('macqcq', $macqcq)
            ->max('id');
        if (isset($idmax)) {
            $model = GiaVtXk::where('id',$idmax)->first();
    ...
    public function nhanhs(Request $request){
        if (Session::has('admin')) {
            ...
            return redirect('xetduyetkekhaigiavtxk?madv='.$model->macqcq);
        }else
            return view('errors.notlogin');
    }
    ...
    public function congbo(Request $request)
    {
        if (Session::has('admin')) {
            ...
            $model->save();
            return redirect('xetduyetkekhaigiavtxk?madv=' . $model->madv_ad);
        } else
            return view('errors.notlogin');
    }
