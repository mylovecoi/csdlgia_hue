...
class KkGiaThanController extends Controller
{
    ...
    public function index(Request $request)
    {
        if (Session::has('admin')) {
            $inputs = $request->all();
            $inputs['url'] = '/kekhaigiathan';
            $m_donvi = getDoanhNghiepNhapLieu(session('admin')->level, 'THAN');
            if (count($m_donvi) == 0) {
                return view('errors.noperm')
                    ->with('url', '')
                    ->with('message', 'Hệ thống chưa có doanh nghiệp kê khai giá than.');
            }
            ...
            return view('manage.kkgia.than.kkgia.kkgiadv.index')
                ...
                ->with('pageTitle', 'Danh sách hồ sơ kê khai giá than');
        } else
            return view('errors.notlogin');
    }
    ...
    public function create(Request $request)
    {
        if (Session::has('admin')) {
            ...
            if ($modellk != null) {
                $modellkct = KkGiaThanCt::where('mahs', $modellk->mahs)->get();
                ...
                KkGiaThanCt::insert($a_dm);
            }

            $modelct = KkGiaThanCt::where('mahs', $inputs['mahs'])->get();
            //            dd($modelct);
    ...
    public function store(Request $request)
    {
        if (Session::has('admin')) {
            ...
            return redirect('kekhaigiathan?&madv=' . $inputs['madv']);
        } else
            return view('errors.notlogin');
    }
    ...
    public function show(Request $request)
    {
        if (Session::has('admin')) {
            ...
            if (strtotime($modelkk->ngayhieuluc) < strtotime('2024-01-01')) {
                return view('manage.kkgia.than.reports.print56')
        ...
        if (Session::has('admin')) {
            if (session('admin')->level == 'DN' || session('admin')->level == 'T' || session('admin')->level == 'H'  || session('admin')->level == 'X') {
                ...
                if (session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X' || $model->madv == session('admin')->madv) {
                    ...
                    return redirect('kekhaigiathan?&madv=' . $model->madv);
                } else
                    return view('errors.perm');
            } else
                return view('errors.perm');
    ...
    public function delete(Request $request)
    {
        if (Session::has('admin')) {
            if (session('admin')->level == 'DN' || session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X') {
                $inputs = $request->all();
                $model = KkGiaThan::where('id', $inputs['iddelete'])
                    ->first();
                if ($model->delete()) {
    ...
    public function kiemtra(Request $request)
    {
        ...
        //dd($request);
        $inputs = $request->all();
        $m_hs = KkGiaThan::where('mahs', $inputs['mahs'])->first();
        if (KiemTraNgayApDung($m_hs->ngayhieuluc, 'than')) {
            $result = array(
                'status' => 'success',
