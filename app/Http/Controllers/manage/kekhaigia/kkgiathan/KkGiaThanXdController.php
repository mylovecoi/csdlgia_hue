...
class KkGiaThanXdController extends Controller
{
    public function index(Request $request)
    {
        if (Session::has('admin')) {
            ...
            switch ($inputs['level']){
                case 'H':{
                    $model = KkGiaThan::where('madv_h', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
                ...
                case 'T':{
                    $model = KkGiaThan::where('madv_t', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
                ...
                case 'ADMIN':{
                    $model = KkGiaThan::where('madv_ad', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
    ...
    public function ttdnkkthan(Request $request){
        $result = array(
            'status' => 'fail',
            'message' => 'error',
        );
        ...
        if(isset($inputs['id'])){
            ...
            $modelhs = KkGiaThan::where('id',$inputs['id'])->first();
    ...
    public function tralai(Request $request){
        if (Session::has('admin')) {
            ...
            return redirect('xetduyetgiathan?madv=' . $inputs['madvtralai']);
    ...
    public function ttnhanhs(Request $request)
    {
        ...
        $inputs = $request->all();
        $modelhs = KkGiaThan::where('mahs', $inputs['mahs'])->first();
        $modeldn = Company::where('madv', $modelhs->madv)->first();
    ...
    public function getsohsnhan($macqcq){
        $idmax = KkGiaThan::wherein('trangthai', ['DD', 'CB', 'HCB'])
            ->max('id');
        if (isset($idmax)) {
            $model = KkGiaThan::where('id',$idmax)->first();
    ...
    public function nhanhs(Request $request){
        if (Session::has('admin')) {
            ...
            return redirect('xetduyetgiathan?madv='.$model->macqcq);
        }else
            return view('errors.notlogin');
    }
    ...
    public function congbo(Request $request)
    {
        if (Session::has('admin')) {
            ...
            $model->save();
            return redirect('xetduyetgiathan?madv=' . $model->madv_ad);
        } else
            return view('errors.notlogin');
    }
