...
class KkGiaCatSanController extends Controller
{
    ...
    public function show(Request $request)
    {
        if (Session::has('admin')) {
            ...
            if (strtotime($modelkk->ngayhieuluc) < strtotime('2024-01-01')) {
                return view('manage.kkgia.catsan.reports.print56')
    ...
    public function chuyen(Request $request)
    {
        if (Session::has('admin')) {
            $inputs = $request->all();
            $model = KkGiaCatSan::where('mahs', $inputs['mahs'])->first();
            if (KkGiaCatSan::where('madv', $model->madv)->where('trangthai', 'CD')->count() > 0) {
                return view('errors.403')
                    ->with('message', 'Doanh nghiệp đang có hồ sơ chờ nhận trên đơn vị chủ quản nên không thể chuyển hồ sơ.')
                    ->with('url', '/kekhaigiacatsan?madv=' . $model->madv)
