...
class KkGiaCatSanBcController extends Controller
{
    public function index(){
        if (Session::has('admin')) {
            $modeldmnghe = DmNgheKd::where('manghe','CATSAN')
                ->first();
            $a_diaban = getDiaBan_Level(\session('admin')->level, \session('admin')->madiaban);
            $m_diaban = dsdiaban::wherein('madiaban', array_keys($a_diaban))->get();
            return view('manage.kkgia.catsan.reports.index')
                ->with('a_diaban', array_column($m_diaban->wherein('level', ['H','T','X'])->toarray(), 'tendiaban', 'madiaban'))
                ->with('pageTitle', 'Báo cáo tổng hợp kê khai giá cát sạn');
        }else
    ...
    public function bc1(Request $request){
        if (Session::has('admin')) {
            $inputs = $request->all();
            $model =  KkGiaCatSan::join('company','company.madv','=','kkgiacatsan.madv')
                ->where('kkgiacatsan.trangthai','DD')
                ->select('kkgiacatsan.*','company.tendn');
            ...
            return view('manage.kkgia.catsan.reports.bc1')
                ->with('model',$model)
                ->with('inputs',$inputs)
                ->with('pageTitle', 'Báo cáo tổng hợp kê khai giá cát sạn');
        }else
    ...
    public function bc2(Request $request){
        if (Session::has('admin')) {
            ...
            $modelct = KkGiaCatSanCt::whereIn('mahs',explode(',',$mahss))->get();

            return view('manage.kkgia.catsan.reports.bc2')
                ->with('model',$model)
                ->with('inputs',$inputs)
