...
class KkGiaCatSanCtController extends Controller
{
    public function store(Request $request){
        ...
        if(isset($inputs['tendvcu'])){
            $modelkkgia = new KkGiaCatSanCt();
            $modelkkgia->create($inputs);
            $model = KkGiaCatSanCt::where('mahs',$inputs['mahs'])->get();
    ...
    public function edit(Request $request){
        ...
        if(isset($inputs['id'])){
            $id = $inputs['id'];
            $model = KkGiaCatSanCt::findOrFail($id);
    ...
    public function update(Request $request){
        ...
        $inputs['giakk'] = getDoubleToDb($inputs['giakk']);
        if(isset($inputs['id'])){
            $modelkkgia = KkGiaCatSanCt::where('id',$inputs['id'])->first();
            $modelkkgia->update($inputs);
            $model = KkGiaCatSanCt::where('mahs',$inputs['mahs'])
                ->get();
            $result['message'] = '<div class="row" id="dsts">';
            $result['message'] .= '<div class="col-md-12">';
            $result['message'] .= '<table class="table table-striped table-bordered table-hover" id="sample_3">';
