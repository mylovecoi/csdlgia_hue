...
class KkGiaCatSanXdController extends Controller
{
    public function index(Request $request)
    {
        if (Session::has('admin')) {
            $inputs = $request->all();
            $inputs['url'] = '/xetduyetkkgiacatsan';
            //lấy địa bàn
            ...
            switch ($inputs['level']) {
                case 'H': {
                        $model = KkGiaCatSan::where('madv_h', $inputs['madv']);
                        if ($inputs['nam'] != 'all')
                ...
                case 'T': {
                        $model = KkGiaCatSan::where('madv_t', $inputs['madv']);
                        if ($inputs['nam'] != 'all')
                ...
                case 'ADMIN': {
                        $model = KkGiaCatSan::where('madv_ad', $inputs['madv']);
                        if ($inputs['nam'] != 'all')
            ...
            return view('manage.kkgia.catsan.kkgia.xetduyet.index')
                ->with('model', $model)
                ->with('inputs', $inputs)
                ...
                ->with('pageTitle', 'Xét duyệt hồ sơ kê khai giá cát sạn');
        } else
            return view('errors.notlogin');
    }

    public function ttdnkkcatsan(Request $request)
    {
        $result = array(
            'status' => 'fail',
            'message' => 'error',
        ...
        if (isset($inputs['id'])) {
            ...
            $modelhs = KkGiaCatSan::where('id', $inputs['id'])->first();
    ...
    public function tralai(Request $request)
    {
        if (Session::has('admin')) {
            ...
            /*dd($inputs);*/
            $inputs['macqcq'] = 'BTL';
            $model = KkGiaCatSan::where('id', $inputs['idtralai'])->first();
            ...
            return redirect('xetduyetkkgiacatsan?madv=' . $inputs['madvtralai']);
        } else
            return view('errors.notlogin');
    }
    ...
    public function ttnhanhs(Request $request)
    {
        ...
        $inputs = $request->all();
        $modelhs = KkGiaCatSan::where('mahs', $inputs['mahs'])->first();
        $modeldn = Company::where('madv', $modelhs->madv)->first();
    ...
    {
        $idmax = KkGiaCatSan::wherein('trangthai', ['DD', 'CB', 'HCB'])
            ->max('id');
        if (isset($idmax)) {
            $model = KkGiaCatSan::where('id', $idmax)->first();
    ...
    public function nhanhs(Request $request)
    {
        if (Session::has('admin')) {
            ...
            return redirect('xetduyetkkgiacatsan?madv=' . $model->macqcq);
        } else
            return view('errors.notlogin');
    }
    ...
    public function congbo(Request $request)
    {
        if (Session::has('admin')) {
            ...
            $model->save();
            return redirect('xetduyetkkgiacatsan?madv=' . $model->madv_ad);
        } else
            return view('errors.notlogin');
    }
    ...
    public function search(Request $request)
    {
        if (Session::has('admin')) {
            ...
            $model = $model->get();
            /*dd($model);*/
            return view('manage.kkgia.catsan.kkgia.timkiem.index')
                ->with('model', $model)
                ->with('inputs', $inputs)
