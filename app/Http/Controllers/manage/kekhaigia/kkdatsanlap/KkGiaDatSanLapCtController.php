...
class KkGiaDatSanLapCtController extends Controller
{
    public function store(Request $request){
        ...
        if(isset($inputs['tendvcu'])){
            $modelkkgia = new KkGiaDatSanLapCt();
            $modelkkgia->create($inputs);
            $model = KkGiaDatSanLapCt::where('mahs',$inputs['mahs'])->get();
    ...
    public function edit(Request $request){
        ...
        if(isset($inputs['id'])){
            $id = $inputs['id'];
            $model = KkGiaDatSanLapCt::findOrFail($id);
    ...
    public function update(Request $request){
        ...
        $inputs['giakk'] = getDoubleToDb($inputs['giakk']);
        if(isset($inputs['id'])){
            $modelkkgia = KkGiaDatSanLapCt::where('id',$inputs['id'])->first();
            $modelkkgia->update($inputs);
            $model = KkGiaDatSanLapCt::where('mahs',$inputs['mahs'])
                ->get();
            $result['message'] = '<div class="row" id="dsts">';
            $result['message'] .= '<div class="col-md-12">';
            $result['message'] .= '<table class="table table-striped table-bordered table-hover" id="sample_3">';
