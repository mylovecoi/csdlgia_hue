...
class GiaOtoNkSxXdController extends Controller
{
    public function index(Request $request)
    {
        if (Session::has('admin')) {
            ...
            switch ($inputs['level']){
                case 'H':{
                    $model = GiaOtoNkSx::where('madv_h', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
                ...
                case 'T':{
                    $model = GiaOtoNkSx::where('madv_t', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
                ...
                case 'ADMIN':{
                    $model = GiaOtoNkSx::where('madv_ad', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
            ...
            return view('manage.kkgia.otonksx.kkgia.xetduyet.index')
                ...
                ->with('pageTitle', 'Xét duyệt hồ sơ kê khai giá ô tô nhập khẩu, sản xuất trong nước');
        } else
            return view('errors.notlogin');
    }

    public function ttdnkkgiaotonksx(Request $request){
        $result = array(
            'status' => 'fail',
            'message' => 'error',
        );
        ...
        if(isset($inputs['id'])){
            ...
            $modelhs = GiaOtoNkSx::where('id',$inputs['id'])->first();
    ...
    public function tralai(Request $request){
        if (Session::has('admin')) {
            ...
            return redirect('xetduyetgiaotonksx?madv=' . $inputs['madvtralai']);
    ...
    public function ttnhanhs(Request $request)
    {
        ...
        $inputs = $request->all();
        $modelhs = GiaOtoNkSx::where('mahs', $inputs['mahs'])->first();
        $modeldn = Company::where('madv', $modelhs->madv)->first();
    ...
    public function nhanhs(Request $request){
        if (Session::has('admin')) {
            ...
            return redirect('xetduyetgiaotonksx?madv='.$model->macqcq);
        }else
            return view('errors.notlogin');
    }
    ...
    public function congbo(Request $request)
    {
        if (Session::has('admin')) {
            ...
            $model->save();
            return redirect('xetduyetgiaotonksx?madv=' . $model->madv_ad);
        } else
            return view('errors.notlogin');
    }
    ...
    public function search(Request $request){
        if (Session::has('admin')) {
            ...
            $model = GiaOtoNkSxCt::join('giaotonksx','giaotonksx.mahs','=','giaotonksxct.mahs')
                ->join('company','company.madv','=','giaotonksx.madv')
                ->select('giaotonksxct.*','company.tendn','giaotonksx.ngayhieuluc')
                ->where('giaotonksx.trangthai','DD');
            if($inputs['mota'] != '')
                $model = $model->where('giaotonksxct.tendvcu','like','%'.$inputs['mota'].'%');
            if($inputs['nam'] != 'all')
                $model = $model->whereYear('giaotonksx.ngayhieuluc',$inputs['nam']);
            $model = $model->get();
            /*dd($model);*/
            return view('manage.kkgia.otonksx.kkgia.timkiem.index')
                ->with('model',$model)
                ->with('inputs',$inputs)
                ->with('pageTitle','Tìm kiếm thông tin kê khai giá ô tô nhập khẩu, sản xuất trong nước');
        }else
            return view('errors.notlogin');
    }
    ...
    public function printf(Request $request){
        if (Session::has('admin')) {
            ...
            $model = $model->get();
            /*dd($model);*/
            return view('manage.kkgia.otonksx.kkgia.timkiem.printf')
                ->with('model',$model)
                ->with('inputs',$inputs)
                ->with('pageTitle','Tìm kiếm thông tin kê khai giá ô tô nhập khẩu, sản xuất trong nước');
        }else
