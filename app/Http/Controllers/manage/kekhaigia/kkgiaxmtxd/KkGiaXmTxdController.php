...
class KkGiaXmTxdController extends Controller
{
    public function ttdn(Request $request)
    {
        if (Session::has('admin')) {
            if (session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X') {
                $inputs = $request->all();
                $modeldmnghe = DmNgheKd::where('manghe', 'XMTXD')
                    ->first();
    ...
    public function create(Request $request)
    {
        if (Session::has('admin')) {
            ...
            $modellk = KkGiaXmTxd::where('madv', $inputs['madv'])
                ->wherein('trangthai', ['DD', 'CB', 'HCB'])
                ->orderby('ngayhieuluc', 'desc')->first();

            if ($modellk != null) {
                $modellkct = KkGiaXmTxdCt::where('mahs', $modellk->mahs)->get();
                ...
                KkGiaXmTxdCt::insert($a_dm);
            }

            $modelct = KkGiaXmTxdCt::where('mahs', $inputs['mahs'])->get();
            //            dd($modelct);
    ...
    public function store(Request $request)
    {
        if (Session::has('admin')) {
            ...
            $model = KkGiaXmTxd::where('mahs', $inputs['mahs'])->first();
            if ($model == null) {
                $inputs['trangthai'] = 'CC';
                $inputs['congbo'] = 'CHUACONGBO';
                KkGiaXmTxd::create($inputs);
            } else {
                $model->update($inputs);
            }
            return redirect('kekhaigiaxmtxd?&madv=' . $inputs['madv']);
        } else
            return view('errors.notlogin');
    }
    ...
    public function edit(Request $request)
    {
        if (Session::has('admin')) {
            ...
            $modelct = KkGiaXmTxdCt::where('mahs', $model->mahs)->get();
            /*dd($modelct);*/
            return view('manage.kkgia.xmtxd.kkgia.kkgiadv.edit')
                ->with('model', $model)
                ->with('modeldn', $modeldn)
        ...
        if (Session::has('admin')) {
            if (session('admin')->level == 'DN' || session('admin')->level == 'T' || session('admin')->level == 'H'  || session('admin')->level == 'X') {
                ...
                if (session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X' || $model->madv == session('admin')->madv) {
                    ...
                    return redirect('kekhaigiaxmtxd?&madv=' . $model->madv);
                } else
                    return view('errors.perm');
            } else
                return view('errors.perm');
    ...
    public function delete(Request $request)
    {
        if (Session::has('admin')) {
            if (session('admin')->level == 'DN' || session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X') {
                $inputs = $request->all();
                $model = KkGiaXmTxd::where('id', $inputs['iddelete'])
                    ->first();
                if ($model->delete()) {
    ...
    public function kiemtra(Request $request)
    {
        ...
        //dd($request);
        $inputs = $request->all();
        $m_hs = KkGiaXmTxd::where('mahs', $inputs['mahs'])->first();
        if (KiemTraNgayApDung($m_hs->ngayhieuluc, 'xmtxd')) {
            $result = array(
                'status' => 'success',
    ...
    public function chuyen(Request $request)
    {
        if (Session::has('admin')) {
            $inputs = $request->all();
            $model = KkGiaXmTxd::where('mahs', $inputs['mahs'])->first();
            if (KkGiaXmTxd::where('madv', $model->madv)->where('trangthai', 'CD')->count() > 0) {
                return view('errors.403')
                    ->with('message', 'Doanh nghiệp đang có hồ sơ chờ nhận trên đơn vị chủ quản nên không thể chuyển hồ sơ.')
                    ->with('url', 'kekhaigiaxmtxd?madv=' . $model->madv)
