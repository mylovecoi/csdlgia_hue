...
class GiaXeMayNkSxBcController extends Controller
{
    public function index(){
        if (Session::has('admin')) {
            $modeldmnghe = DmNgheKd::where('manghe','XEMAY')
                ->first();
            ...
            return view('manage.kkgia.xemaynksx.reports.index')
                ...
                ->with('pageTitle', 'Báo cáo tổng hợp kê khai giá xe máy nhập khẩu, sản xuất trong nước');
        }else
    ...
    public function bc1(Request $request){
        if (Session::has('admin')) {
            $inputs = $request->all();
            $model =  GiaXeMayNkSx::join('company','company.madv','=','giaxemaynksx.madv')
    ...
    public function bc2(Request $request){
        if (Session::has('admin')) {
            ...
            $modelct = GiaXeMayNkSxCt::whereIn('mahs',explode(',',$mahss))->get();
