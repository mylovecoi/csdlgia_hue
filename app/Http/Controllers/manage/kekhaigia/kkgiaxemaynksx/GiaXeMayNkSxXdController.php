...
class GiaXeMayNkSxXdController extends Controller
{
    public function index(Request $request)
    {
        if (Session::has('admin')) {
            $inputs = $request->all();
            $inputs['url'] = '/xetduyetgiaxemaynksx';
            //lấy địa bàn
            ...
            switch ($inputs['level']){
                case 'H':{
                    $model = GiaXeMayNkSx::where('madv_h', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
                ...
                case 'T':{
                    $model = GiaXeMayNkSx::where('madv_t', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
                ...
                case 'ADMIN':{
                    $model = GiaXeMayNkSx::where('madv_ad', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
    ...
    public function ttdnkkgiaxemaynksx(Request $request){
        $result = array(
            'status' => 'fail',
            'message' => 'error',
        );
        ...
        if(isset($inputs['id'])){
            ...
            $modelhs = GiaXeMayNkSx::where('id',$inputs['id'])->first();
    ...
    public function tralai(Request $request){
        if (Session::has('admin')) {
            ...
            return redirect('xetduyetgiaxemaynksx?madv=' . $inputs['madvtralai']);
    ...
    public function ttnhanhs(Request $request)
    {
        ...
        $inputs = $request->all();
        $modelhs = GiaXeMayNkSx::where('mahs', $inputs['mahs'])->first();
        $modeldn = Company::where('madv', $modelhs->madv)->first();
    ...
    public function nhanhs(Request $request){
        if (Session::has('admin')) {
            ...
            return redirect('xetduyetgiaxemaynksx?madv='.$model->macqcq);
        }else
            return view('errors.notlogin');
    }
    ...
    public function congbo(Request $request)
    {
        if (Session::has('admin')) {
            ...
            $model->save();
            return redirect('xetduyetgiaxemaynksx?madv=' . $model->madv_ad);
        } else
            return view('errors.notlogin');
    }
    ...
    public function search(Request $request){
        if (Session::has('admin')) {
            ...
            $model = GiaXeMayNkSxCt::join('giaxemaynksx','giaxemaynksx.mahs','=','giaxemaynksxct.mahs')
                ->join('company','company.madv','=','giaxemaynksx.madv')
                ->select('giaxemaynksxct.*','company.tendn','giaxemaynksx.ngayhieuluc')
                ->where('giaxemaynksx.trangthai','DD');
            if($inputs['mota'] != '')
                $model = $model->where('giaxemaynksxct.tendvcu','like','%'.$inputs['mota'].'%');
            if($inputs['nam'] != 'all')
                $model = $model->whereYear('giaxemaynksx.ngayhieuluc',$inputs['nam']);
            $model = $model->get();
            /*dd($model);*/
    ...
    public function printf(Request $request){
        if (Session::has('admin')) {
            ...
            $model = $model->get();
            /*dd($model);*/
            return view('manage.kkgia.xemaynksx.kkgia.timkiem.printf')
                ->with('model',$model)
                ->with('inputs',$inputs)
                ->with('pageTitle','Tìm kiếm thông tin kê khai giá xe máy nhập khẩu, sản xuất trong nước');
        }else
