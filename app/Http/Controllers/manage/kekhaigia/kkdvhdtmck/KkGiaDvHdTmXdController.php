...
class KkGiaDvHdTmXdController extends Controller
{
    public function index(Request $request)
    {
        if (Session::has('admin')) {
            $inputs = $request->all();
            $inputs['url'] = '/xetduyetkkgiadvhdtm';
            //lấy địa bàn
            ...
            switch ($inputs['level']){
                case 'H':{
                    $model = KkGiaDvHdTm::where('madv_h', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
                ...
                case 'T':{
                    $model = KkGiaDvHdTm::where('madv_t', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
                ...
                case 'ADMIN':{
                    $model = KkGiaDvHdTm::where('madv_ad', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
            ...
            return view('manage.kkgia.dvhdtm.kkgia.xetduyet.index')
                ...
                ->with('pageTitle', 'Xét duyệt hồ sơ kê khai giá dịch vụ thương mại');
        } else
            return view('errors.notlogin');
    }

    public function ttdndvhdtm(Request $request){
        $result = array(
            'status' => 'fail',
            'message' => 'error',
        );
        ...
        if(isset($inputs['id'])){
            ...
            $modelhs = KkGiaDvHdTm::where('id',$inputs['id'])->first();
    ...
    public function tralai(Request $request){
        if (Session::has('admin')) {
            ...
            return redirect('xetduyetkkgiadvhdtm?madv=' . $inputs['madvtralai']);
    ...
    public function ttnhanhs(Request $request)
    {
        ...
        $inputs = $request->all();
        $modelhs = KkGiaDvHdTm::where('mahs', $inputs['mahs'])->first();
        $modeldn = Company::where('madv', $modelhs->madv)->first();
    ...
    public function getsohsnhan($macqcq){
        $idmax = KkGiaDvHdTm::wherein('trangthai', ['DD', 'CB', 'HCB'])
            ->max('id');
        if (isset($idmax)) {
            $model = KkGiaDvHdTm::where('id',$idmax)->first();
    ...
    public function nhanhs(Request $request){
        if (Session::has('admin')) {
            ...
            return redirect('xetduyetkkgiadvhdtm?madv='.$model->macqcq);
        }else
            return view('errors.notlogin');
    }
    ...
    public function search(Request $request){
        if (Session::has('admin')) {
            ...
            $model = KkGiaDvHdTmCt::join('kkgiadvhdtm','kkgiadvhdtm.mahs','=','kkgiadvhdtmct.mahs')
                ->join('company','company.madv','=','kkgiadvhdtm.madv')
                ->select('kkgiadvhdtmct.*','company.tendn','kkgiadvhdtm.ngayhieuluc')
                ->where('kkgiadvhdtm.trangthai','DD');
            if($inputs['mota'] != '')
                $model = $model->where('kkgiadvhdtmct.tendvcu','like','%'.$inputs['mota'].'%');
            if($inputs['nam'] != 'all')
                $model = $model->whereYear('kkgiadvhdtm.ngayhieuluc',$inputs['nam']);
            $model = $model->get();
            /*dd($model);*/
            return view('manage.kkgia.dvhdtm.kkgia.timkiem.index')
                ->with('model',$model)
                ->with('inputs',$inputs)
                ->with('pageTitle','Tìm kiếm thông tin kê khai giá dịch vụ thương mại');
        }else
            return view('errors.notlogin');
    }
    ...
    public function printf(Request $request){
        if (Session::has('admin')) {
            ...
            $model = $model->get();
            /*dd($model);*/
            return view('manage.kkgia.dvhdtm.kkgia.timkiem.printf')
                ->with('model',$model)
                ->with('inputs',$inputs)
                ->with('pageTitle','Tìm kiếm thông tin kê khai giá dịch vụ thương mại');
        }else
