...
class KkGiaDvHdTmController extends Controller
{
    public function index(Request $request){
        if (Session::has('admin')) {
            $inputs = $request->all();
            $inputs['url'] = '/kekhaigiadvhdtm';
            $m_donvi = getDoanhNghiepNhapLieu(session('admin')->level, 'DVHDTMCK');
            if(count($m_donvi) == 0){
                return view('errors.noperm')
                    ->with('url','')
                    ->with('message','Hệ thống chưa có doanh nghiệp kê khai giá dịch vụ thương mại.');
            }
            ...
            $model = KkGiaDvHdTm::where('madv', $inputs['madv'])
            ...
            return view('manage.kkgia.dvhdtm.kkgia.kkgiadv.index')
                ...
                ->with('pageTitle', 'Danh sách hồ sơ kê khai giá dịch vụ thương mại.');
        } else
    ...
    public function create(Request $request)
    {
        if (Session::has('admin')) {
            ...
            $modellk = KkGiaDvHdTm::where('madv', $inputs['madv'])
                ->wherein('trangthai', ['DD', 'CB', 'HCB'])
                ->orderby('ngayhieuluc', 'desc')->first();

            if ($modellk != null) {
                $modellkct = KkGiaDvHdTmCt::where('mahs', $modellk->mahs)->get();
                ...
                KkGiaDvHdTmCt::insert($a_dm);
            }

            $modelct = KkGiaDvHdTmCt::where('mahs', $inputs['mahs'])->get();
//            dd($model);
            ...
            return view('manage.kkgia.dvhdtm.kkgia.kkgiadv.edit')
                ...
                ->with('inputs', $inputs)
                ->with('pageTitle', 'Kê khai giá dịch vụ thương mại thêm mới');
    ...
    public function store(Request $request){
        if (Session::has('admin')) {
            ...
            if ($model == null) {
                $inputs['trangthai'] = 'CC';
                KkGiaDvHdTm::create($inputs);
            } else {
                $model->update($inputs);
            }
            return redirect('kekhaigiadvhdtm?&madv='.$inputs['madv']);
    ...
    public function show(Request $request){
        if (Session::has('admin')) {
            ...
            $modelkkct = KkGiaDvHdTmCt::where('mahs',$modelkk->mahs)->get();
//            dd($modelkkct);
            ...
            return view('manage.kkgia.dvhdtm.reports.print')
                ...
                ->with('pageTitle','Kê khai giá dịch vụ thương mại');
    ...
    public function edit(Request $request){
        if (Session::has('admin')) {
            ...
            $modelct = KkGiaDvHdTmCt::where('mahs', $model->mahs)->get();
            return view('manage.kkgia.dvhdtm.kkgia.kkgiadv.edit')
                ->with('model', $model)
                ->with('modeldn', $modeldn)
                ->with('modelct', $modelct)
                ->with('pageTitle', 'Kê khai giá dịch vụ thương mại chỉnh sửa');
        } else
            return view('errors.notlogin');
    }
        ...
        if (Session::has('admin')) {
            if (session('admin')->level == 'DN' || session('admin')->level == 'T' || session('admin')->level == 'H'  || session('admin')->level == 'X') {
                ...
                if (session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X' || $model->madv == session('admin')->madv) {
                    ...
                    return redirect('kekhaigiadvhdtm?&madv=' . $model->madv);
                } else
    ...
    public function delete(Request $request){
        if (Session::has('admin')) {
            if (session('admin')->level == 'DN' || session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X') {
                $inputs = $request->all();
                $model = KkGiaDvHdTm::where('id',$inputs['iddelete'])
                    ->first();
                if($model->delete()){
    ...
    public function kiemtra(Request $request){
        ...
        //dd($request);
        $inputs = $request->all();
        $m_hs = KkGiaDvHdTm::where('mahs',$inputs['mahs'])->first();
        if(KiemTraNgayApDung($m_hs->ngayhieuluc,'dvhdtmck')){
            $result = array(
                'status' => 'success',
