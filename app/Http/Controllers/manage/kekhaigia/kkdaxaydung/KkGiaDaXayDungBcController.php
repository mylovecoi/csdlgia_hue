...
class KkGiaDaXayDungBcController extends Controller
{
    public function index(){
        if (Session::has('admin')) {
            $modeldmnghe = DmNgheKd::where('manghe','DAXAYDUNG')
                ->first();
            $a_diaban = getDiaBan_Level(\session('admin')->level, \session('admin')->madiaban);
            $m_diaban = dsdiaban::wherein('madiaban', array_keys($a_diaban))->get();
            return view('manage.kkgia.daxaydung.reports.index')
                ->with('a_diaban', array_column($m_diaban->wherein('level', ['H','T','X'])->toarray(), 'tendiaban', 'madiaban'))
                ->with('pageTitle', 'Báo cáo tổng hợp kê khai giá đá xây dựng');
        }else
    ...
    public function bc1(Request $request){
        if (Session::has('admin')) {
            $inputs = $request->all();
            $model =  KkGiaDaXayDung::join('company','company.madv','=','kkgiadaxaydung.madv')
                ->where('kkgiadaxaydung.trangthai','DD')
                ->select('kkgiadaxaydung.*','company.tendn');
            ...
            return view('manage.kkgia.daxaydung.reports.bc1')
                ->with('model',$model)
                ->with('inputs',$inputs)
                ->with('pageTitle', 'Báo cáo tổng hợp kê khai giá đá xây dựng');
        }else
    ...
    public function bc2(Request $request){
        if (Session::has('admin')) {
            ...
            $modelct = KkGiaDaXayDungCt::whereIn('mahs',explode(',',$mahss))->get();

            return view('manage.kkgia.daxaydung.reports.bc2')
                ->with('model',$model)
                ->with('inputs',$inputs)
