...
class KkGiaKcbTnBcController extends Controller
{
    public function index(){
        if (Session::has('admin')) {
            $modeldmnghe = DmNgheKd::where('manghe','KCBTN')
                ->first();
            ...
            return view('manage.kkgia.kcbtn.reports.index')
                ...
                ->with('pageTitle', 'Báo cáo tổng hợp kê khai giá khám chữa bệnh');
        }else
    ...
    public function bc1(Request $request){
        if (Session::has('admin')) {
            $inputs = $request->all();
            $model =  KkGiaKcbTn::join('company','company.madv','=','kkgiakcbtn.madv')
    ...
    public function bc2(Request $request){
        if (Session::has('admin')) {
            ...
            $modelct = KkGiaKcbTnCt::whereIn('mahs',explode(',',$mahss))->get();
