...
class KkGiaKcbTnController extends Controller
{
    public function index(Request $request)
    {
        if (Session::has('admin')) {
            $inputs = $request->all();
            $inputs['url'] = '/kekhaigiakcbtn';
            $m_donvi = getDoanhNghiepNhapLieu(session('admin')->level, 'KCBTN');
            if (count($m_donvi) == 0) {
                return view('errors.noperm')
                    ->with('url', '')
                    ->with('message', 'Hệ thống chưa có doanh nghiệp kê khai giá khám chữa bệnh tư nhân.');
            }
            ...
            $model = KkGiaKcbTn::where('madv', $inputs['madv'])
            ...
            $m_donvi_th = getDonViTongHop_dn('kcbtn', session('admin')->level, session('admin')->madiaban);
            ...
            return view('manage.kkgia.kcbtn.kkgia.kkgiadv.index')
                ...
                ->with('pageTitle', 'Danh sách hồ sơ kê khai giá khám chữa bệnh tư nhân.');
        } else
    ...
    public function create(Request $request)
    {
        if (Session::has('admin')) {
            ...
            $modellk = KkGiaKcbTn::where('madv', $inputs['madv'])
                ->wherein('trangthai', ['DD', 'CB', 'HCB'])
                ->orderby('ngayhieuluc', 'desc')->first();

            if ($modellk != null) {
                $modellkct = KkGiaKcbTnCt::where('mahs', $modellk->mahs)->get();
                ...
                KkGiaKcbTnCt::insert($a_dm);
            }

            $modelct = KkGiaKcbTnCt::where('mahs', $inputs['mahs'])->get();
            //            dd($model);
            ...
            return view('manage.kkgia.kcbtn.kkgia.kkgiadv.edit')
                ...
                ->with('inputs', $inputs)
                ->with('pageTitle', 'Kê khai giá khám chữa bệnh thêm mới');
        } else
            return view('errors.notlogin');
    }
    ...
    public function store(Request $request)
    {
        if (Session::has('admin')) {
            ...
            $model = KkGiaKcbTn::where('mahs', $inputs['mahs'])->first();
            if ($model == null) {
                $inputs['trangthai'] = 'CC';
                KkGiaKcbTn::create($inputs);
            } else {
                $model->update($inputs);
            }
            return redirect('kekhaigiakcbtn?&madv=' . $inputs['madv']);
        } else
            return view('errors.notlogin');
    }
    ...
    public function edit(Request $request)
    {
        if (Session::has('admin')) {
            ...
            return view('manage.kkgia.kcbtn.kkgia.kkgiadv.edit')
                ...
                ->with('modeldn', $modeldn)
                ->with('modelct', $modelct)
                ->with('pageTitle', 'Kê khai giá khám chữa bệnh chỉnh sửa');
        } else
            return view('errors.notlogin');
    }
            ...
            if (session('admin')->level == 'DN' || session('admin')->level == 'T' || session('admin')->level == 'H'  || session('admin')->level == 'X') {
                ...
                if (session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X' || $model->madv == session('admin')->madv) {
                    ...
                    if ($model->update($inputs)) {
                        $modelct = KkGiaKcbTnCt::where('mahs', $inputs['mahs'])
                            ->update(['trangthai' => 'XD']);
                    }
                    return redirect('kekhaigiakcbtn?&madv=' . $model->madv);
                } else
    ...
    public function delete(Request $request)
    {
        if (Session::has('admin')) {
            if (session('admin')->level == 'DN' || session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X') {
                $inputs = $request->all();
                $model = KkGiaKcbTn::where('id', $inputs['iddelete'])
                    ->first();
                if ($model->delete()) {
    ...
    public function kiemtra(Request $request)
    {
        ...
        //dd($request);
        $inputs = $request->all();
        $m_hs = KkGiaKcbTn::where('mahs', $inputs['mahs'])->first();
        if (KiemTraNgayApDung($m_hs->ngayhieuluc, 'kcbtn')) {
            $result = array(
                'status' => 'success',
    ...
    public function chuyen(Request $request)
    {
        if (Session::has('admin')) {
            $inputs = $request->all();
            $model = KkGiaKcbTn::where('mahs', $inputs['mahs'])->first();
            if (KkGiaKcbTn::where('madv', $model->madv)->where('trangthai', 'CD')->count() > 0) {
                return view('errors.403')
                    ->with('message', 'Doanh nghiệp đang có hồ sơ chờ nhận trên đơn vị chủ quản nên không thể chuyển hồ sơ.')
                    ->with('url', '/kekhaigiakcbtn?madv=' . $model->madv)
    ...
    public function create_excel(Request $request)
    {
        if (Session::has('admin')) {
            ...
            return view('manage.kkgia.kcbtn.kkgia.kkgiadv.edit')
                ...
                ->with('modelct', $modelct)
                ->with('inputs', $inputs)
                ->with('pageTitle', 'Kê khai giá khám chữa bệnh tư nhân thêm mới');
        } else
            return view('errors.notlogin');
    }
}
