...
class GiaNuocShController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (Session::has('admin')) {
            ...
            $m_donvi = getDonViNhapLieu(session('admin')->level,'gianuocsh');
            if (count($m_donvi) == null) {
                $message = 'Chưa có đơn vị nào được phân quyền nhập liệu cho chức năng: ' . session('admin')['a_chucnang']['gianuocsh']
...
////            $result['message'] .= '<p ><a target = "_blank" href = "' . url('/data/gianuocsachsinhhoat/' . $model->ipf1) . '">' . $model->ipf1 . '</a ></p >';
...
////        $result['status'] = 'success';
        ...
        if (isset($model->ipf2)) {
            $result['message'] .= '<div class="col-md-6" ><div class="form-group">';
            $result['message'] .= '<label class="control-label" > File đính kèm</label>';
            $result['message'] .= '<p><a target = "_blank" href = "' . url('/data/gianuocsachsinhhoat/' . $model->ipf2) . '">' . $model->ipf2 . '</a ></p>';
            $result['message'] .= '</div></div>';
        }
        if (isset($model->ipf3)) {
            $result['message'] .= '<div class="col-md-6" ><div class="form-group">';
            $result['message'] .= '<label class="control-label" > File đính kèm</label>';
            $result['message'] .= '<p><a target = "_blank" href = "' . url('/data/gianuocsachsinhhoat/' . $model->ipf3) . '">' . $model->ipf3 . '</a ></p>';
            $result['message'] .= '</div></div>';
        }
        if (isset($model->ipf4)) {
            $result['message'] .= '<div class="col-md-6" ><div class="form-group">';
            $result['message'] .= '<label class="control-label" > File đính kèm</label>';
            $result['message'] .= '<p><a target = "_blank" href = "' . url('/data/gianuocsachsinhhoat/' . $model->ipf4) . '">' . $model->ipf4 . '</a ></p>';
            $result['message'] .= '</div></div>';
        }
        if (isset($model->ipf5)) {
            $result['message'] .= '<div class="col-md-6" ><div class="form-group">';
            $result['message'] .= '<label class="control-label" > File đính kèm</label>';
            $result['message'] .= '<p><a target = "_blank" href = "' . url('/data/gianuocsachsinhhoat/' . $model->ipf5) . '">' . $model->ipf5 . '</a ></p>';
            $result['message'] .= '</div></div>';
        }
        $result['message'] .= '</div>';
        $result['status'] = 'success';
