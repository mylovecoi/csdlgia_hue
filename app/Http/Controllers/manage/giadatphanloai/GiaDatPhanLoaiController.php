...
class GiaDatPhanLoaiController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (Session::has('admin')) {
            ...
            $m_donvi = getDonViNhapLieu(session('admin')->level, 'giadatpl');
            if (count($m_donvi) == null) {
                $message = 'Chưa có đơn vị nào được phân quyền nhập liệu cho chức năng: ' . session('admin')['a_chucnang']['giadatpl']
    ...
    public function create(Request $request)
    {
        ...
        if (Session::has('admin')) {
            ...
            $modelct = GiaDatPhanLoaiCt::where('id', -1)->get();
            //dd($a_khuvuc);
    ...
    public function create_excel(Request $request)
    {
        if (Session::has('admin')) {
            ...
            for ($i = $inputs['tudong']; $i <= $inputs['dendong']; $i++) {
                if (
                    !isset($data[$i][$inputs['khuvuc']]) || !isset($data[$i][$inputs['maloaidat']]) ||
    ...
    function getHoSo($inputs)
    {
        ...
        switch ($inputs['level']) {
            ...
            default: { //mặc định lấy đơn vị nhâp liệu
                    $model = GiaDatPhanLoai::where('madv', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
                        $model = $model->whereYear('thoidiem', $inputs['nam']);
                    break;
