...
use App\Model\manage\dinhgia\phichuyengia\phichuyengia;
use App\Model\manage\dinhgia\phichuyengia\phichuyengiact;
use App\Model\system\dsdiaban;
use App\Model\system\dsdonvi;
use App\Model\system\view_dsdiaban_donvi;
use App\Model\view\view_phichuyengia;
...
class phichuyengiaController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (Session::has('admin')) {
            ...
            $m_donvi_th = getDonViTongHop('phichuyengia',\session('admin')->level, \session('admin')->madiaban);
            $inputs['madiaban'] = $inputs['madiaban'] ?? $m_diaban->first()->madiaban;
            $inputs['madv'] = $inputs['madv'] ?? $m_donvi->first()->madv;
            $inputs['nam'] = $inputs['nam'] ?? 'all';

            //lấy thông tin đơn vị
            $model = phichuyengia::where('madv', $inputs['madv']);
            if ($inputs['nam'] != 'all')
                $model = $model->whereYear('thoidiem', $inputs['nam']);

            return view('manage.phichuyengia.kekhai.index')
    ...
    public function create(Request $request)
    {
        if (Session::has('admin')) {
            ...
            DB::statement("DELETE FROM phichuyengiact WHERE mahs not in (SELECT mahs FROM phichuyengia where madv='" . $inputs['madv'] . "')");
            $model = new phichuyengia();
            ...
            phichuyengiact::insert($a_dm);
            $inputs['url'] = '/phichuyengia';
            $modelct = phichuyengiact::where('mahs', $model->mahs)->get();
            return view('manage.phichuyengia.kekhai.edit')
                ->with('model', $model)
    ...
    public function update(Request $request){
        if(Session::has('admin')) {
            ...
            if($model == null){
                ...
                phichuyengia::create($inputs);
            }else{
                $model->update($inputs);
            }

            return redirect('phichuyengia/danhsach?madv='.$inputs['madv']);
        }else
            return view('errors.notlogin');
    }
    ...
    public function chuyenhs(Request $request)
    {
        ...
        if (Session::has('admin')) {
            ...
            $model->save();
            return redirect('phichuyengia/danhsach?madv=' . $model->madv);
        } else
            return view('errors.notlogin');
    }
    ...
    public function destroy(Request $request){
        if(Session::has('admin')){
            $inputs = $request->all();
            $model = phichuyengia::where('mahs', $inputs['mahs'])->first();
            phichuyengiact::where('mahs',$model->mahs)->delete();
            $model->delete();
            return redirect('phichuyengia/danhsach?madv='.$model->madv);
    ...
    public function xetduyet(Request $request)
    {
        ...
        if (Session::has('admin')) {
            ...
            switch ($inputs['level']){
                case 'H':{
                    $model = phichuyengia::where('madv_h', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
                ...
                case 'T':{
                    $model = phichuyengia::where('madv_t', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
                ...
                case 'ADMIN':{
                    $model = phichuyengia::where('madv_ad', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
    ...
    public function chuyenxd(Request $request)
    {
        ...
        if (Session::has('admin')) {
            ...
            //dd($model);
            $model->save();
            return redirect('phichuyengia/xetduyet?madv=' . $inputs['madv']);
        } else
            return view('errors.notlogin');
    }
    ...
    public function congbo(Request $request)
    {
        if (Session::has('admin')) {
            ...
            $model->save();
            return redirect('phichuyengia/xetduyet?madv=' . $model->madv_ad);
        } else
            return view('errors.notlogin');
    }
    //</editor-fold>
    ...
    public function ketxuat(Request $request){
        if(Session::has('admin')){
            ...
            $a_dm = array_column(dmphichuyengia::all()->toarray(),
                'tengia', 'maso');
            return view('manage.phichuyengia.reports.baocao')
                ->with('model',$model)
                ->with('a_dm',$a_dm)
                ->with('m_donvi',$m_donvi)
                ->with('m_diaban',$m_diaban)
    ...
    function getHoSo($inputs)
    {
        ...
        switch ($inputs['level']) {
            case 'H':
            {
                $model = view_phichuyengia::where('madv_h', $inputs['madv']);
                if ($inputs['nam'] != 'all')
                    $model = $model->whereYear('thoidiem_h', $inputs['nam']);
                break;
            }
            case 'T':
            {
                $model = view_phichuyengia::where('madv_t', $inputs['madv']);
                if ($inputs['nam'] != 'all')
                    $model = $model->whereYear('thoidiem_t', $inputs['nam']);
                break;
            }
            case 'ADMIN':
            {
                $model = view_phichuyengia::where('madv_ad', $inputs['madv']);
                if ($inputs['nam'] != 'all')
                    $model = $model->whereYear('thoidiem_ad', $inputs['nam']);
                break;
            }
            ...
            {//mặc định lấy đơn vị nhâp liệu
                $model = view_phichuyengia::where('madv', $inputs['madv']);
                if ($inputs['nam'] != 'all')
                    $model = $model->whereYear('thoidiem', $inputs['nam']);
                break;
            }
    ...
    public function timkiem(){
        if(Session::has('admin')){
            ...
            return view('manage.phichuyengia.timkiem.index')
                ->with('m_diaban',$m_diaban)
                ->with('m_donvi',$m_donvi)
                ->with('a_dm',$a_dm)
    ...
    public function ketquatk(Request $request){
        if(Session::has('admin')){
            ...
            $inputs = $request->all();
            $m_donvi = getDonViTimKiem(session('admin')->level, \session('admin')->madiaban);
            $model = view_phichuyengia::wherein('madv',array_column($m_donvi->toarray(),'madv'))->get();
            //dd($model);
            ...
            //dd($model);
            return view('manage.phichuyengia.timkiem.result')
                ->with('model',$model)
