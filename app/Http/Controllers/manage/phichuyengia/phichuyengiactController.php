...
class phichuyengiactController extends Controller
{
    public function store(Request $request){
        ...
        $inputs = $request->all();
        $inputs['mucgia'] = getMoneyToDb($inputs['mucgia']);
        $m_chk = phichuyengiact::where('maso',$inputs['maso'])->where('mahs',$inputs['mahs'])->first();
        if($m_chk == null){
            phichuyengiact::create($inputs);
        }else{
            $m_chk->update($inputs);
        }
        $model = phichuyengiact::where('mahs',$inputs['mahs'])->get();
    ...
    public function show(Request $request){
        ...
        $inputs = $request->all();
        $id = $inputs['id'];
        $model = phichuyengiact::findOrFail($id);
        die($model);
    }
    ...
    public function destroy(Request $request){
        ...
        $inputs = $request->all();
        phichuyengiact::where('id',$inputs['id'])->first();
        $model = phichuyengiact::where('mahs',$inputs['mahs'])->get();
        $result = $this->return_spdv($model);
        die(json_encode($result));
    }
    ...
    public function return_spdv($model)
    {
        ...
        if (count($model) > 0) {
            ...
            foreach ($model as $key => $tents) {
                ...
                $result['message'] .= '<td class="active" style="font-weight: bold">' . ($a_dm[$tents->maso] ?? '') . '</td>';
                $result['message'] .= '<td style="text-align: right;font-weight: bold">' . number_format($tents->mucgia) . '</td>';
                $result['message'] .= '<td>' .
