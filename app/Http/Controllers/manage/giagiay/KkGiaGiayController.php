...
class KkGiaGiayController extends Controller
{
    public function ttdn(Request $request){
        if (Session::has('admin')) {
            ...
            if (session('admin')->level == 'T' || session('admin')->level == 'H' || session('admin')->level == 'X') {
                $inputs = $request->all();
                $inputs['mh'] = DmNgheKd::where('manghe','GIAY')
                    ->first()->tennghe;
                $modeldmnghe = DmNgheKd::where('manghe','GIAY')
                    ->first();
                ...
                return view('manage.kkgia.dkg.kekhaigiagiay.kekhai.ttdn')
                    ->with('model', $model)
    ...
    public function index(Request $request){
        if (Session::has('admin')) {
            ...
            $m_donvi_th = getDonViTongHop_dn('giagiay',session('admin')->level, session('admin')->madiaban);
            ...
            $m_lvkd = CompanyLvCc::where('madv', $inputs['madv'])
                ->wherein('manghe',array_column($m_giay->toarray(),'manghe'))->get();
            //lấy danh mục nghề theo đơn vị đăng ký
            $m_giay = $m_giay->wherein('manghe',array_column($m_lvkd->toarray(),'manghe'));
            $model = KkGiaGiay::where('madv',$inputs['madv'])->get();
            if ($inputs['nam'] != 'all')
                $model = KkGiaGiay::where('madv',$inputs['madv'])
                    ->whereYear('ngayhieuluc', $inputs['nam'])
                    ->get();

            return view('manage.giagiay.kekhai.index')
                ...
                ->with('a_nghe',array_column($m_giay->toarray(),'tennghe','manghe'))
                ->with('a_donvi_th',array_column($m_donvi_th->toarray(),'tendv','madv'))
                ->with('a_diaban_th',array_column($m_donvi_th->toarray(),'tendiaban','madiaban'))
                ->with('pageTitle', 'Danh sách hồ sơ giá kê khai giấy');
    ...
    public function create(Request $request){
        if (Session::has('admin')) {
            ...
            DB::statement("DELETE FROM kkgiagiayct WHERE mahs not in (SELECT mahs FROM kkgiagiay where madv='" . $inputs['madv'] . "')");

            //lấy hồ sơ liền kề
            $hslk = KkGiaGiay::where('trangthai', 'HT')
            ...
            if($hslk != null){
                ...
                $m_ct = KkGiaGiayCt::where('mahs', $hslk->mahs)->get();
                $a_ct = array();
                foreach ($m_ct as $ct) {
                ...
                KkGiaGiayCt::insert($a_ct);
            }

            $inputs['url'] = '/giagiay';
            return view('manage.giagiay.kekhai.create')
                ->with('model', $model)
                ->with('model_ct', nullValue())
                ->with('m_nghe', $m_nghe)
                ->with('inputs', $inputs)
                ->with('m_dn', $m_dn)
                ->with('pageTitle', 'Giá kê khai giấy');
        }
    }
    ...
    public function store(Request $request){
        if (Session::has('admin')) {
            $inputs = $request->all();
            $model = KkGiaGiay::where('mahs',$inputs['mahs'])->first();
            if($model == null){
                $inputs['phanloai'] = 'DK';
                $inputs['trangthai'] = 'CC';
                $inputs['congbo'] = 'CHUACONGBO';
                KkGiaGiay::create($inputs);
            }else{
                $model->update($inputs);
            }
            return redirect('giagiay/danhsach?madv='.$inputs['madv']);
    ...
    public function show(Request $request){
        if (Session::has('admin')) {
            ...
            $modelkkct = KkGiaGiayCt::where('mahs',$modelkk->mahs)->get();
            $modelcqcq = view_dsdiaban_donvi::where('madv', $modelkk->macqcq)->first();
            return view('manage.giagiay.baocao.print')
    ...
    public function edit(Request $request){
        if (Session::has('admin')) {
            ...
            $m_nghe = DmNgheKd::where('manghe', 'GIAY')->first();
            ...
            return view('manage.giagiay.kekhai.create')
                ...
                ->with('inputs', $inputs)
                ->with('m_dn', $m_dn)
                ->with('pageTitle', 'Chỉnh sửa hồ sơ giá giấy');
        }else
            return view('errors.notlogin');
    }
    ...
    public function destroy(Request $request){
        if (Session::has('admin')) {
            ...
            $model = KkGiaGiay::where('mahs',$inputs['mahs'])->first();
            if($model->delete()){
                KkGiaGiayCt::where('mahs',$model->mahs)->delete();
            }
            return redirect('giagiay/danhsach?madv='.$model->madv);
        }else
            return view('errors.notlogin');
    }
    ...
    public function timkiem(){
        if (Session::has('admin')) {
            ...
            $m_donvi = getDoanhNghiep(session('admin')->level, session('admin')->madiaban);
            $m_diaban = dsdiaban::wherein('madiaban', a_unique(array_column($m_donvi->toArray(),'madiaban')))->get();
            $m_dm = view_dmnganhnghe::where('manghe', 'GIAY')->get();

            return view('manage.giagiay.timkiem.index')
                ->with('inputs', $inputs)
                ->with('m_diaban', $m_diaban)
                ->with('a_dm', array_column($m_dm->toarray(),'tennghe','manghe'))
                ->with('m_donvi', $m_donvi)
                ->with('a_phanloai',  array('DK'=>'Đăng ký giá','KK'=>'Kê khai giá'))
                ->with('pageTitle', 'Tìm kiếm hồ sơ giá giấy');
    ...
    public function ketquatk(Request $request){
        if (Session::has('admin')) {
            ...
            $model =  KkGiaGiayCt::join('kkgiagiay','kkgiagiay.mahs', '=', 'kkgiagiayct.mahs')
                ...
                ->select('kkgiagiayct.*','kkgiagiay.ngayhieuluc','company.tendn');
            ...
            if (getDayVn($inputs['ngayapdung_tu']) != '') {
                $model = $model->where('kkgiagiay.ngayhieuluc', '>=', $inputs['ngayapdung_tu']);
            }

            if (getDayVn($inputs['ngayapdung_den']) != '') {
                $model = $model->where('kkgiagiay.ngayhieuluc', '<=', $inputs['ngayapdung_den']);
            }

            $model = $model->where('kkgiagiayct.dongia', '>=', chkDbl($inputs['dongia_tu']));
            if (chkDbl($inputs['dongia_den']) > 0) {
                $model = $model->where('kkgiagiayct.dongia', '<=', chkDbl($inputs['dongia_den']));
            }
            $model = $model->get();

            $a_dm = array_column(view_dmnganhnghe::where('manghe', 'GIAY')->get()->toArray(), 'tennghe', 'manghe');
            return view('manage.giagiay.timkiem.result')
                ->with('model', $model)
                ->with('inputs', $inputs)
                ->with('a_dm', $a_dm)
                ->with('pageTitle', 'Tìm kiếm thông tin giá giấy');
        } else
            return view('errors.notlogin');
    }
}
