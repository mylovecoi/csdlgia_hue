...
use App\Model\view\view_giaspdvtoida;
use Illuminate\Http\Request;
use App\Http\Controllers\Controller;
use Illuminate\Support\Facades\Session;

class giaspdvtoidaController extends Controller
{
    public function index(Request $request)
    {
        ...
        if (Session::has('admin')) {
            ...
            if (count($m_donvi) == null) {
                $message = 'Chưa có đơn vị nào được phân quyền nhập liệu cho chức năng: ' . session('admin')['a_chucnang']['giaspdvtoida']
            ...
            $model = giaspdvtoida::where('madv', $inputs['madv']);
            if ($inputs['nam'] != 'all')
                $model = $model->whereYear('thoidiem', $inputs['nam']);
            //Ko dung $inputs['madiaban'] do $m_diaban chứa cả T, H
            //$a_ts = array_column(giaspdvtoidadm::all()->toArray(),'tenspdv','maspdv');

            //dd($inputs);
            return view('manage.dinhgia.giaspdvtoida.kekhai.index')
    ...
    public function show_dk(Request $request)
    {
        ...
        if (isset($model->ipf1)) {
            ...
            $result['message'] .= '<p ><a target = "_blank" href = "' . url('/data/giaspdvtoida/' . $model->ipf1) . '">' . $model->ipf1 . '</a ></p >';
    ...
    public function create(Request $request){
        if(Session::has('admin')){
            ...
            $modelct = giaspdvtoida_ct::where('id', -1)->get();
...
//                'tenspdv','maspdv');
    ...
    public function xetduyet(Request $request)
    {
        ...
        if (Session::has('admin')) {
            ...
            switch ($inputs['level']){
                case 'H':{
                    $model = giaspdvtoida::where('madv_h', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
                ...
                case 'T':{
                    $model = giaspdvtoida::where('madv_t', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
                ...
                case 'ADMIN':{
                    $model = giaspdvtoida::where('madv_ad', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
            ...
            //dd($model);
            return view('manage.dinhgia.giaspdvtoida.xetduyet.index')
                ->with('model', $model)
                ->with('inputs', $inputs)
                ->with('m_diaban', $m_diaban)
                ->with('a_diaban', array_column($m_diaban->where('level', 'H')->toarray(), 'tendiaban', 'madiaban'))
    ...
    public function chuyenxd(Request $request)
    {
        ...
        if (Session::has('admin')) {
            ...
            //dd($model);
            $model->save();
            return redirect('giaspdvtoida/xetduyet?madv=' . $inputs['madv']);
        } else
            return view('errors.notlogin');
    }
    ...
    public function congbo(Request $request)
    {
        if (Session::has('admin')) {
            ...
            $model->save();
            return redirect('giaspdvtoida/xetduyet?madv=' . $model->madv_ad);
        } else
            return view('errors.notlogin');
    }
    //</editor-fold>
    ...
    function getHoSo($inputs)
    {
        ...
        switch ($inputs['level']) {
            case 'H':
            {
                $model = view_giaspdvtoida::where('madv_h', $inputs['madv']);
                if ($inputs['nam'] != 'all')
                    $model = $model->whereYear('thoidiem_h', $inputs['nam']);
                break;
            }
            case 'T':
            {
                $model = view_giaspdvtoida::where('madv_t', $inputs['madv']);
                if ($inputs['nam'] != 'all')
                    $model = $model->whereYear('thoidiem_t', $inputs['nam']);
                break;
            }
            case 'ADMIN':
            {
                $model = view_giaspdvtoida::where('madv_ad', $inputs['madv']);
                if ($inputs['nam'] != 'all')
                    $model = $model->whereYear('thoidiem_ad', $inputs['nam']);
                break;
            }
            ...
            {//mặc định lấy đơn vị nhâp liệu
                $model = view_giaspdvtoida::where('madv', $inputs['madv']);
                if ($inputs['nam'] != 'all')
                    $model = $model->whereYear('thoidiem', $inputs['nam']);
                break;
            }
    ...
    public function timkiem(){
        if(Session::has('admin')){
            ...
            $inputs['url'] = '/giaspdvtoida';
            return view('manage.dinhgia.giaspdvtoida.timkiem.index')
                ->with('m_diaban',$m_diaban)
                ->with('m_donvi',$m_donvi)
                ->with('a_dm',$a_ts)
                ->with('inputs',$inputs)
    ...
    public function ketquatk(Request $request){
        if(Session::has('admin')){
            ...
            //dd($model);
            $a_ts = array_column(giaspdvtoida_dm::all()->toArray(),'tenspdv','maspdv');
            return view('manage.dinhgia.giaspdvtoida.timkiem.result')
                ->with('model',$model)
