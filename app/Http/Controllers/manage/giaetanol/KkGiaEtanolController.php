    ...
    public function ketquatk(Request $request){
        if (Session::has('admin')) {
            ...
            $model =  KkGiaEtanolCt::join('kkgiaetanol','kkgiaetanol.mahs', '=', 'kkgiaetanolct.mahs')
                ...
                ->select('kkgiaetanolct.*','kkgiaetanol.ngayhieuluc','company.tendn');
            ...
            if (getDayVn($inputs['ngayapdung_tu']) != '') {
                $model = $model->where('kkgiaetanol.ngayhieuluc', '>=', $inputs['ngayapdung_tu']);
            }

            if (getDayVn($inputs['ngayapdung_den']) != '') {
                $model = $model->where('kkgiaetanol.ngayhieuluc', '<=', $inputs['ngayapdung_den']);
            }

            $model = $model->where('kkgiaetanolct.dongia', '>=', chkDbl($inputs['dongia_tu']));
            if (chkDbl($inputs['dongia_den']) > 0) {
                $model = $model->where('kkgiaetanolct.dongia', '<=', chkDbl($inputs['dongia_den']));
            }
            $model = $model->get();
