...
namespace App\Http\Controllers\manage\khunggiadat;

use App\DiaBanHd;
use App\Model\manage\dinhgia\giagdbatdongsan\GiaGdBatDongSan;
use App\Model\manage\dinhgia\khunggiadat\khunggiadat;
...
class khunggiadatController extends Controller
{
    public function index(Request $request){
        if (Session::has('admin')) {
            $inputs = $request->all();
            $inputs['url'] = '/khunggiadat';
            //lấy địa bàn
            $a_diaban = getDiaBan_Level(\session('admin')->level, \session('admin')->madiaban);
            $m_diaban = dsdiaban::wherein('madiaban', array_keys($a_diaban))->get();
            $m_donvi = getDonViNhapLieu(session('admin')->level);
            $m_donvi_th = getDonViTongHop('khunggiadat',\session('admin')->level, \session('admin')->madiaban);
            $inputs['madiaban'] = $inputs['madiaban'] ?? $m_diaban->first()->madiaban;
            $inputs['madv'] = $inputs['madv'] ?? $m_donvi->first()->madv;
            $inputs['nam'] = $inputs['nam'] ?? 'all';

            //lấy thông tin đơn vị
            $model = khunggiadat::where('madv', $inputs['madv']);
            if ($inputs['nam'] != 'all')
                $model = $model->whereYear('thoidiem', $inputs['nam']);
            //dd($inputs);
            return view('manage.dinhgia.khunggiadat.kekhai.index')
    ...
    public function create(Request $request)
    {
        if (Session::has('admin')) {
            ...
            $a_xa = array_column(dsxaphuong::where('madiaban', array_key_first($a_diaban) ?? '')->get()->toarray(),
                'tenxp', 'maxp');
            return view('manage.dinhgia.khunggiadat.kekhai.edit')
                ->with('model', $model)
                ->with('inputs', $inputs)
                ->with('a_xp', $a_xa)
                ->with('a_diaban', $a_diaban)
                ->with('pageTitle', 'Hồ sơ khung giá đất');
        } else
            return view('errors.notlogin');
    }
    ...
    public function store(Request $request){
        if(Session::has('admin')){
            ...
            if(isset($inputs['ipf1'])){
                ...
                $ipf1->move(public_path() . '/data/khunggiadat/', $name);
                $inputs['ipf1']= $name;
            }

            $model = khunggiadat::where('mahs',$inputs['mahs'])->first();
            if($model == null){
                $inputs['trangthai'] = 'CHT';
                khunggiadat::create($inputs);
            }else{
                $model->update($inputs);
            }
            return redirect('/khunggiadat/danhsach?madv='.$inputs['madv']);
        }else
            return view('errors.notlogin');
    }
    ...
    public function show(Request $request)
    {
        ...
        if (isset($model->ipf1)) {
            ...
            $result['message'] .= '<p ><a target = "_blank" href = "' . url('/data/khunggiadat/' . $model->ipf1) . '">' . $model->ipf1 . '</a ></p >';
    ...
    public function destroy(Request $request){
        if (Session::has('admin')) {
            ...
            $model = khunggiadat::where('mahs',$inputs['mahs'])->first();
            $model->delete();
            return redirect('khunggiadat/danhsach?madv='.$model->madv);
        }else
            return view('errors.notlogin');
    }
    ...
    public function xetduyet(Request $request)
    {
        ...
        if (Session::has('admin')) {
            ...
            switch ($inputs['level']){
                case 'H':{
                    $model = khunggiadat::where('madv_h', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
                ...
                case 'T':{
                    $model = khunggiadat::where('madv_t', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
                ...
                case 'ADMIN':{
                    $model = khunggiadat::where('madv_ad', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
            ...
            //dd($model);
            return view('manage.dinhgia.khunggiadat.xetduyet.index')
                ->with('model', $model)
                ->with('inputs', $inputs)
                ->with('m_diaban', $m_diaban)
    ...
    public function chuyenxd(Request $request)
    {
        ...
        if (Session::has('admin')) {
            ...
            //dd($model);
            $model->save();
            return redirect('khunggiadat/xetduyet?madv=' . $inputs['madv']);
        } else
            return view('errors.notlogin');
    }
    ...
    public function congbo(Request $request)
    {
        if (Session::has('admin')) {
            ...
            $model->save();
            return redirect('khunggiadat/xetduyet?madv=' . $model->madv_ad);
        } else
            return view('errors.notlogin');
    }
    //</editor-fold>
    ...
    public function timkiem(){
        if(Session::has('admin')){
            ...
            return view('manage.dinhgia.khunggiadat.timkiem.index')
                ->with('m_diaban',$m_diaban)
                ->with('m_donvi',$m_donvi)
                //->with('a_dm',$a_dm)
    ...
    public function ketquatk(Request $request){
        if(Session::has('admin')){
            ...
            $inputs = $request->all();
            $m_donvi = getDonViTimKiem(session('admin')->level, \session('admin')->madiaban);
            $model = khunggiadat::wherein('madv',array_column($m_donvi->toarray(),'madv'));
            //dd($inputs);
            ...
            //dd($model);
            return view('manage.dinhgia.khunggiadat.timkiem.result')
                ->with('model',$model->get())
