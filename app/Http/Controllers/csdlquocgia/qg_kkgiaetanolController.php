...
class qg_kkgiaetanolController extends Controller
{
    ...
    {
        $inputs = $request->all();
        $inputs['url'] = '/csdlquocgia/qg_kkgiaetanol/congbo_hoso';
        $m_donvi = getDonViXetDuyet('SSA');
        $inputs['nam'] = $inputs['nam'] ?? 'all';
        $inputs['madv'] = $inputs['madv'] ?? $m_donvi->first()->madv;
        $inputs['truyendulieu'] = $inputs['truyendulieu'] ?? 'all';
        $model = KkGiaEtanol::where('macqcq', $inputs['madv']);
        if ($inputs['nam'] != 'all')
            $model = $model->whereYear('thoidiem', $inputs['nam']);
        if ($inputs['truyendulieu'] != 'all')
            $model = $model->where('truyendulieu', $inputs['truyendulieu']);

        return view('csdlquocgia.qg_kkgiaetanol.truyenhoso.congbo')
            ->with('model', $model->get())
            ->with('inputs', $inputs)
            ->with('m_donvi', $m_donvi)
            ->with('pageTitle', 'Công bố hồ sơ kê khai giá xăng dầu');
    }
}
