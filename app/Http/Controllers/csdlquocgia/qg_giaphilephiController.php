...
class qg_giaphilephiController extends Controller
{
    public function nhandanhmuc(Request $request)
    {
        ...
        $inputs['url'] = '/csdlquocgia/qg_giaphilephi/nhandanhmuc';
        return view('csdlquocgia.qg_giaphilephi.nhandanhmuc.index')
            ->with('model', $model)
            ->with('inputs', $inputs)
            ->with('pageTitle', 'Danh mục Giá lệ phí trước bạ');
    }
    ...
    public function nhanhoso(Request $request)
    {
        if (Session::has('admin')) {
            $inputs = $request->all();
            $inputs['url'] = '/csdlquocgia/qg_giaphilephi/nhanhoso';
            $a_diaban = getDiaBan_Level(\session('admin')->level, \session('admin')->madiaban);
            $m_diaban = dsdiaban::wherein('madiaban', array_keys($a_diaban))->get();
            $m_donvi = getDonViNhapLieu(session('admin')->level, 'giaphilephi');
            if (count($m_donvi) == null) {
                $message = 'Chưa có đơn vị nào được phân quyền nhập liệu cho chức năng: ' . session('admin')['a_chucnang']['giaphilephi']
            ...
            $model = PhiLePhi::where('madv', $inputs['madv']);
            if ($inputs['nam'] != 'all')
                $model = $model->whereYear('thoidiem', $inputs['nam']);
            $a_dm = array_column(DmPhiLePhi::all()->toArray(), 'manhom', 'tennhom');
            return view('csdlquocgia.qg_giaphilephi.nhanhoso.index')
                ...
                ->with('pageTitle', 'Nhận hồ sơ Giá lệ phí trước bạ');
        } else
            return view('errors.notlogin');
    }
    ...
    public function chuyenhs(Request $request)
    {
        if (Session::has('admin')) {
            ...
            $model->save();
            return redirect('/csdlquocgia/qg_giaphilephi/nhanhoso?madv=' . $model->madv);
        } else
            return view('errors.notlogin');
    }
    ...
    public function truyendanhmuc(Request $request)
    {
        ...
        $model = DmPhiLePhi::all();
        $inputs['url'] = '/csdlquocgia/qg_giaphilephi/danhmuc';
        $inputs['truyendulieu'] = $inputs['truyendulieu'] ?? 'all';
        if ($inputs['truyendulieu'] != 'all')
            $model = $model->where('truyendulieu', $inputs['truyendulieu']);
        return view('csdlquocgia.qg_giaphilephi.truyendanhmuc.index')
            ->with('model', $model)
            ->with('inputs', $inputs)
            ->with('pageTitle', 'Truyền danh mục Giá lệ phí trước bạ');
    }
    ...
    public function capnhatdanhmuc(Request $request)
    {
        ...
        $model->update($inputs);
        return redirect('/csdlquocgia/qg_giaphilephi/danhmuc?truyendulieu=' . $inputs['truyendulieu']);
    }
    ...
    public function truyenhoso(Request $request)
    {
        ...
        $inputs = $request->all();
        $inputs['url'] = '/csdlquocgia/qg_giaphilephi/hoso';
        $a_diaban = getDiaBan_Level(\session('admin')->level, \session('admin')->madiaban);
        $m_diaban = dsdiaban::wherein('madiaban', array_keys($a_diaban))->get();
        ...
        switch ($inputs['level']) {
            case 'H': {
                    $model = PhiLePhi::where('madv_h', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
            ...
            case 'T': {
                    $model = PhiLePhi::where('madv_t', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
            ...
            case 'ADMIN': {
                    $model = PhiLePhi::where('madv_ad', $inputs['madv']);
                    if ($inputs['nam'] != 'all')
        ...
        return view('csdlquocgia.qg_giaphilephi.truyenhoso.index')
            ->with('model', $model)
            ->with('inputs', $inputs)
            ->with('m_diaban', $m_diaban)
            ...
            ->with('a_donvi_th', array_column($m_donvi_th->toarray(), 'tendv', 'madv'))
            ->with('a_diaban_th', array_column($m_donvi_th->toarray(), 'tendiaban', 'madiaban'))
            ->with('pageTitle', 'Truyền hồ sơ Giá lệ phí trước bạ');
    }
    ...
    public function capnhathoso(Request $request)
    {
        ...
        $model = PhiLePhi::where('mahs', $inputs['mahs'])->first();
        $model->update($inputs);
        return redirect('/csdlquocgia/qg_giaphilephi/hoso?truyendulieu=' . $inputs['truyendulieu']);
    }
    ...
    public function congbo_danhmuc(Request $request)
    {
        $inputs = $request->all();
        $inputs['truyendulieu'] = $inputs['truyendulieu'] ?? 'all';
        $inputs['url'] = '/csdlquocgia/qg_giaphilephi/congbo_danhmuc';
        $model = DmPhiLePhi::all();
        if ($inputs['truyendulieu'] != 'all')
            $model = $model->where('truyendulieu', $inputs['truyendulieu']);
        return view('csdlquocgia.qg_giaphilephi.truyendanhmuc.congbo')
            ->with('model', $model)
            ->with('inputs', $inputs)
            ->with('pageTitle', 'Công bố danh mục Giá lệ phí trước bạ');
    }

    public function congbo_hoso(Request $request)
    {
        $inputs = $request->all();
        $inputs['url'] = '/csdlquocgia/qg_giaphilephi/congbo_hoso';
        $m_donvi = getDonViNhapLieu('ADMIN', 'giaphilephi');
        $inputs['nam'] = $inputs['nam'] ?? 'all';
        $inputs['madv'] = $inputs['madv'] ?? $m_donvi->first()->madv;
        $inputs['truyendulieu'] = $inputs['truyendulieu'] ?? 'all';
        $model = PhiLePhi::where('madv', $inputs['madv']);
        ...
        return view('csdlquocgia.qg_giaphilephi.truyenhoso.congbo')
            ->with('model', $model->get())
            ->with('inputs', $inputs)
            ->with('m_donvi', $m_donvi)
            ->with('pageTitle', 'Công bố hồ sơ kê khai Giá lệ phí trước bạ');
    }
}
