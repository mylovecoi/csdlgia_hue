    ...
    public function store(Request $request){
        if(Session::has('admin')){
            ...
            if($model == null){
                ...
                PhiLePhi::create($inputs);
            }else{
                $model->update($inputs);
            }

            return redirect('giaphilephi/danhsach?madv='.$inputs['madv']);
        }else
            return view('errors.notlogin');
    }
...
//            $a_dm = array_column(DmPhiLePhi::all()->toArray(), 'tennhom', 'manhom');
    ...
    public function destroy(Request $request){
        if(Session::has('admin')){
            ...
            $model = PhiLePhi::where('mahs',$inputs['mahs'])->first();
            $model->delete();
            PhiLePhiCt::where('mahs',$model->mahs)->delete();
...
//    }
    ...
    public function xetduyet(Request $request)
    {
        ...
        if (Session::has('admin')) {
            ...
            //dd($model);
            return view('manage.dinhgia.philephi.xetduyet.index')
                ->with('model', $model)
                ->with('inputs', $inputs)
                ->with('m_diaban', $m_diaban)
                ->with('a_diaban', array_column($m_diaban->where('level', 'H')->toarray(), 'tendiaban', 'madiaban'))
    ...
    public function chuyenxd(Request $request)
    {
        ...
        if (Session::has('admin')) {
            ...
            //dd($model);
            $model->save();
            return redirect('giaphilephi/xetduyet?madv=' . $inputs['madv']);
        } else
            return view('errors.notlogin');
    }
    ...
    public function congbo(Request $request)
    {
        if (Session::has('admin')) {
            ...
            $model->save();
            return redirect('giaphilephi/xetduyet?madv=' . $model->madv_ad);
        } else
            return view('errors.notlogin');
    }
    //</editor-fold>
    ...
    public function ketxuat(Request $request){
        if(Session::has('admin')){
            ...
            $a_dm = array_column(DmPhiLePhi::all()->toArray(),'tennhom','manhom');
            return view('manage.dinhgia.philephi.reports.baocao')
                ->with('model',$model)
                ->with('m_donvi',$m_donvi)
                ->with('m_diaban',$m_diaban)
                ->with('a_dm',$a_dm)
    ...
    function getHoSo($inputs)
    {
        ...
        switch ($inputs['level']) {
            case 'H':
            {
                $model = view_giaphilephi::where('madv_h', $inputs['madv']);
                if ($inputs['nam'] != 'all')
                    $model = $model->whereYear('thoidiem_h', $inputs['nam']);
                break;
            }
            case 'T':
            {
                $model = view_giaphilephi::where('madv_t', $inputs['madv']);
                if ($inputs['nam'] != 'all')
                    $model = $model->whereYear('thoidiem_t', $inputs['nam']);
                break;
            }
            case 'ADMIN':
            {
                $model = view_giaphilephi::where('madv_ad', $inputs['madv']);
                if ($inputs['nam'] != 'all')
                    $model = $model->whereYear('thoidiem_ad', $inputs['nam']);
                break;
            }
            ...
            {//mặc định lấy đơn vị nhâp liệu
                $model = view_giaphilephi::where('madv', $inputs['madv']);
                if ($inputs['nam'] != 'all')
                    $model = $model->whereYear('thoidiem', $inputs['nam']);
                break;
            }
    ...
    public function ketquatk(Request $request){
        if(Session::has('admin')){
            ...
            $inputs = $request->all();
            $m_donvi = getDonViTimKiem(session('admin')->level, \session('admin')->madiaban);
            $model = view_giaphilephi::wherein('madv',array_column($m_donvi->toarray(),'madv'));
            //dd($inputs);
            ...
            $model = $model->where('mucthutu','>=',chkDbl($inputs['giatri_tu']));
            if(chkDbl($inputs['giatri_den']) > 0){
                $model = $model->where('mucthutu','<=',chkDbl($inputs['giatri_den']));
            }
            //dd($model);
            $a_dm = array_column(DmPhiLePhi::all()->toArray(),'tennhom','manhom');
            return view('manage.dinhgia.philephi.timkiem.result')
                ->with('model',$model->get())
